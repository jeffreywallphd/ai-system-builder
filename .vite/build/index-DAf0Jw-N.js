...
for (path$1 of paths) {
  try {
    sharp$3 = commonjsRequire(path$1);
    break;
  } catch (err) {
    errors.push(err);
  }
}
if (sharp$3 && path$1.startsWith("@img/sharp-linux-x64") && !sharp$3._isUsingX64V2()) {
...
function _getBackgroundColourOption(value) {
  if (is$3.object(value) || is$3.string(value) && value.length >= 3 && value.length <= 200) {
...
function tiff(options) {
  if (is$1.object(options)) {
    ...
    if (is$1.defined(options.bigtiff)) {
