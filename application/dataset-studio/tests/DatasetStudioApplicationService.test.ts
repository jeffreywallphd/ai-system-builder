...
import { DatasetStudioApplicationService } from "../DatasetStudioApplicationService";
import { DatasetStudioIdentity } from "../../../domain/dataset-studio/DatasetStudioDomain";
...
describe("DatasetStudioApplicationService", () => {
  it("authors dataset drafts with atomic dataset taxonomy and shared contract/provenance defaults", async () => {
    ...
    expect(ensure.studio.id).toBe(DatasetStudioIdentity.defaultStudioId);
    expect(created.draft.metadata.taxonomy?.structuralKind).toBe("atomic");
    expect(created.draft.metadata.taxonomy?.semanticRole).toBe("dataset");
    ...
    expect(created.draft.metadata.tags).toEqual(["dataset", "customer-support"]);
  });

  it("reuses shared lifecycle/version flow when publishing dataset drafts", async () => {
    const repository = new InMemoryStudioShellRepository();
    ...
    const published = await service.publishDatasetDraft({
      sessionId: ensure.session.id,
    ...
    expect(published.version.versionId).toBe("dataset-version-1");
