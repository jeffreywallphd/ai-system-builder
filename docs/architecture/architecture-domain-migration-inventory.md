---
title: Architecture Domain Migration Inventory
...
# Architecture Domain Migration Inventory

## Purpose

Create a practical migration map from the current flat architecture corpus into the target domain folders so migration stories can move content deliberately and without loss.

## Inventory Summary

- Story scope: `4.2.1`
- Source inventory scope: `docs/architecture/*.md` (human docs only, AI companions follow companion parity)
...
These source documents should be treated as first-pass migration anchors.
...
Each row maps a major source family or content block to a target domain destination and file role.
...
The following high-value docs are mixed enough that direct move-only migration would lose clarity:
...
- Leave link stubs from old architecture paths during migration batches until references are updated.

## Migration Execution Notes

1. Migrate by mapping unit, not by single-file opportunistic edits.
2. For each migrated unit, update both `.md` and `.ai.md` companions in the same change.
3. Keep one canonical destination per contract and replace duplicates with links.
4. Add `## Related ADRs` on destination docs whenever decisions constrain the contract.
5. After each batch, update domain `references/README.md` indexes and context packs that route to moved docs.
