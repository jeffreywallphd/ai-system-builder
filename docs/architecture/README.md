...
- The repository contains **two composition stories**: the generic DI bootstrap in `infrastructure/composition/` and the renderer-specific manual composition in `ui/composition/createUiDependencies.ts`. The current docs treat both as first-class because that is how the code works today, but the long-term architecture would likely benefit from converging them into one clearer composition model.
