...
- For authoritative scheduling visibility projections across run list/detail/status/queue admin-operational surfaces, including effective-priority context, defer rationale, and placement outcomes, read [`run-orchestration-scheduling-visibility-projections.md`](./run-orchestration-scheduling-visibility-projections.md).
- For supported scheduling administrative controls (deferred-run re-evaluation plus stale-reservation visibility/release), including authorization, audit, and no-unsafe-override guardrails, read [`run-orchestration-scheduling-admin-controls.md`](./run-orchestration-scheduling-admin-controls.md).
...
- For canonical realtime publication of scheduling/arbitration decisions (priority selection, deferral, reservation conflict, assignment materialization, and requeue), read [`run-orchestration-scheduling-realtime-event-publication.md`](./run-orchestration-scheduling-realtime-event-publication.md).
- For production scheduler observability diagnostics (structured logs, decision counters, defer/no-placement metrics, reservation conflict markers, and centralized sensitive-data redaction), read [`run-orchestration-scheduling-observability-metrics-and-redaction.md`](./run-orchestration-scheduling-observability-metrics-and-redaction.md).
- For deployment-profile scheduling policy seams (profile-context resolution and profile-aware rule-set construction without placeholder profile toggles), read [`run-orchestration-scheduling-deployment-profile-policy-seams.md`](./run-orchestration-scheduling-deployment-profile-policy-seams.md).
...
- For authoritative run discovery, get-by-id/list query behavior, history-oriented filtering (owner/system/status/time/recent activity/completion), and query-level authorization/hint projection posture, read [`run-orchestration-query-and-history-discovery.md`](./run-orchestration-query-and-history-discovery.md).
- For durable authoritative image-run persistence and lifecycle/status-history recording structure (schema, repository behavior, and migration impact), read [`run-orchestration-image-run-persistence-and-status-history.md`](./run-orchestration-image-run-persistence-and-status-history.md).
...
- For reusable submission readiness evaluation and blocking validation posture before queue admission, read [`run-submission-readiness-blocking-validation.md`](./run-submission-readiness-blocking-validation.md).
...
- For the explicit auth-first Electron startup split boundary (pre-login vs post-login vs on-demand), read [`desktop-auth-first-startup-boundary.md`](./desktop-auth-first-startup-boundary.md).
- For the Story B.1 auth-only startup boundary contract and route-family pre-login classification inventory, read [`auth-only-server-startup-contract.md`](./auth-only-server-startup-contract.md) and [`authoritative-route-family-pre-login-inventory.md`](./authoritative-route-family-pre-login-inventory.md).
...
- For Feature 20.2 deployment-policy durable storage architecture for active profile selection, override history/provenance, effective metadata persistence, and repository boundaries, read [`deployment-profile-policy-persistence-and-repositories.md`](./deployment-profile-policy-persistence-and-repositories.md).
...
- For Feature 20.2 startup/bootstrap deployment-policy resolution in authoritative host composition, including deterministic active-profile fallback and invalid-persisted-state failure behavior, read [`deployment-profile-policy-startup-bootstrap-resolution.md`](./deployment-profile-policy-startup-bootstrap-resolution.md).
- For Feature 20.2 authoritative policy-read APIs that expose active profile, effective state, presets/families metadata, and override provenance to admin/system consumers, read [`deployment-profile-policy-authoritative-read-apis.md`](./deployment-profile-policy-authoritative-read-apis.md).
- For Feature 20.2 authoritative policy-write APIs that expose active profile mutation and typed override administration through canonical server-side control-plane workflows, read [`deployment-profile-policy-authoritative-write-apis.md`](./deployment-profile-policy-authoritative-write-apis.md).
- For Feature 20.3 admin policy-inspection read-model projections and production UI composition for active profile/preset comparison/grouped effective-value provenance, read [`deployment-profile-policy-admin-ui-read-models.md`](./deployment-profile-policy-admin-ui-read-models.md).
- For Feature 20.3.3 policy explainability metadata, family impact summaries, and governance-sensitivity warning presentation in admin policy surfaces, read [`deployment-profile-policy-explainability-and-impact-summaries.md`](./deployment-profile-policy-explainability-and-impact-summaries.md).
- For Feature 20.2 integrated bootstrap/read/write/validation/audit/consumption workflow guidance and explicit deferred-integration limits, read [`deployment-profile-policy-persistence-api-integration-baseline.md`](./deployment-profile-policy-persistence-api-integration-baseline.md).
- For Feature 20.2 deployment-policy mutation governance event hooks and audit/operational sink boundaries, read [`deployment-profile-policy-audit-operational-governance-hooks.md`](./deployment-profile-policy-audit-operational-governance-hooks.md).
...
- For shared actor/workspace/resource/session/device/node reference normalization and authoritative-service normalization workflow, read [`audit-reference-normalization-layer.md`](./audit-reference-normalization-layer.md).
- For authoritative application-layer audit recording service behavior, source-oriented recording ports, and baseline cross-feature wiring, read [`audit-authoritative-recording-service-and-ports.md`](./audit-authoritative-recording-service-and-ports.md).
- For audit taxonomy extension workflow, capture boundaries, prohibited patterns, and prohibited ledger content rules, read [`audit-taxonomy-capture-boundaries-and-extension-rules.md`](./audit-taxonomy-capture-boundaries-and-extension-rules.md).
- For durable canonical audit ledger persistence schema/repository behavior and append-oriented replay-safe query indexing posture, read [`audit-durable-ledger-persistence-and-repositories.md`](./audit-durable-ledger-persistence-and-repositories.md).
- For canonical audit write/read architecture with append invariants, access-control enforcement, redacted detail projection, linkage/correlation retrieval, and retention-lifecycle metadata seams, read [`audit-ledger-persistence-query-and-access-control-architecture.md`](./audit-ledger-persistence-query-and-access-control-architecture.md).
- For contributor implementation workflow when adding or extending canonical audit event types, read [`../audit-governance-contributor-guide.md`](../audit-governance-contributor-guide.md).
...
- For Feature 1 image-asset ingestion completion baseline, end-to-end readiness verification, explicit limits, and Feature 2 prerequisite notes, read [`image-asset-feature-1-final-baseline.md`](./image-asset-feature-1-final-baseline.md).
- For Feature 2 / Epic 2.3 Story 2.3.5 durable workflow/system definition persistence architecture (schema, repositories, replay safety, and migration integration), read [`image-workflow-system-persistence-and-repositories.md`](./image-workflow-system-persistence-and-repositories.md).
- For Feature 2 final baseline completion state, authoritative studio authoring flow, supported-workflow enforcement, and downstream Feature 3/4/6 dependencies, read [`image-workflow-feature-2-final-baseline.md`](./image-workflow-feature-2-final-baseline.md).
- For Feature 3 final baseline on ComfyUI execution adapter completion state, architectural boundaries, supported coverage, and Feature 4/5/6 integration dependencies, read [`image-manipulation-feature-3-final-baseline.md`](./image-manipulation-feature-3-final-baseline.md).
- For Feature 4 / Epic 4.1 Story 4.1.3 image-run repository and orchestration service ports (authoritative persistence, readiness, queueing, dispatch handoff, update ingestion, cancellation, and output notifications), read [`image-run-orchestration-application-ports.md`](./image-run-orchestration-application-ports.md).
- For Feature 4 / Epic 4.1 Story 4.1.5 authoritative run-orchestration posture for image manipulation (authoritative submission, validation, lifecycle normalization, execution handoff, and strict no-direct-studio-to-backend boundaries), read [`image-run-feature-4-epic-4.1-authoritative-orchestration-posture.md`](./image-run-feature-4-epic-4.1-authoritative-orchestration-posture.md).
- For Feature 5 / Epic 5.1 Story 5.1.2 execution-node capability and compatibility contracts used for image workflow/run routing eligibility (hard incompatibility, soft advisory, and transient availability outcomes), read [`execution-node-capability-compatibility-contracts.md`](./execution-node-capability-compatibility-contracts.md).
- For Feature 5 / Epic 5.1 Story 5.1.3 execution-node repository and management application ports for registration/query/health/capability refresh, availability controls, and selection hints, read [`execution-node-repository-and-management-application-ports.md`](./execution-node-repository-and-management-application-ports.md).
- For Feature 5 / Epic 5.1 Story 5.1.4 shared execution-node management/readiness API DTO and schema contracts covering node inventory, readiness, eligibility, and backend availability surfaces, read [`execution-node-management-readiness-api-contracts.md`](./execution-node-management-readiness-api-contracts.md).
- For Feature 5 / Epic 5.1 Story 5.1.5 node-based execution posture for the image manipulation slice (authoritative node eligibility, backend readiness surfaces, orchestration-facing routing seams, and ComfyUI adapter boundary ownership), read [`image-manipulation-node-based-execution-posture.md`](./image-manipulation-node-based-execution-posture.md).
- For Feature 5 / Epic 5.2 Story 5.2.2 concrete persistence for execution-node records and durable operational status/capability history, read [`execution-node-persistence-and-status-history.md`](./execution-node-persistence-and-status-history.md).
- For Feature 5 / Epic 5.2 Story 5.2.5 authoritative enable/disable/suppress execution-node availability override controls and probe-independent operational availability behavior, read [`execution-node-repository-and-management-application-ports.md`](./execution-node-repository-and-management-application-ports.md).
- For Feature 5 / Epic 5.3 Story 5.3.1 run-to-node eligibility evaluation rules and structured eligibility outputs for image runs, read [`execution-node-run-to-node-eligibility-evaluation.md`](./execution-node-run-to-node-eligibility-evaluation.md).
- For Feature 5 / Epic 5.3 Story 5.3.2 initial deterministic backend-node selection strategy for image runs, including structured no-selection outcomes and scheduler-extension seams, read [`execution-node-initial-selection-strategy.md`](./execution-node-initial-selection-strategy.md).
- For Feature 5 / Epic 5.3 Story 5.3.4 authoritative execution-readiness checks that include current node eligibility/availability for specific run requests, read [`execution-readiness-node-availability-checks.md`](./execution-readiness-node-availability-checks.md).
- For Feature 5 / Epic 5.4 Story 5.4.1 authoritative execution-node management/readiness API endpoints for inventory, availability controls, readiness/eligibility checks, and backend posture reads, read [`execution-node-management-readiness-authoritative-apis.md`](./execution-node-management-readiness-authoritative-apis.md).
- For Feature 5 / Epic 5.4 Story 5.4.4 final completion verification and follow-on integration guidance for node-based execution and backend management, read [`image-manipulation-feature-5-final-baseline.md`](./image-manipulation-feature-5-final-baseline.md).
- For Feature 4 / Epic 4.2 Story 4.2.5 concrete persistence implementation for authoritative image runs and durable lifecycle/status history, read [`run-orchestration-image-run-persistence-and-status-history.md`](./run-orchestration-image-run-persistence-and-status-history.md).
- For Feature 4 / Epic 4.4 Story 4.4.4 final completion verification and follow-on integration guidance for authoritative image run orchestration, read [`image-run-feature-4-final-baseline.md`](./image-run-feature-4-final-baseline.md).
- For Feature 6 / Epic 6.1 Story 6.1.5 authoritative result-persistence, preview-safe retrieval, and lineage boundary posture for image manipulation, read [`generated-result-authoritative-persistence-preview-lineage-posture.md`](./generated-result-authoritative-persistence-preview-lineage-posture.md).
- For Feature 6 generated-result domain and API contract foundations used by Story 6.1.5, read [`generated-result-asset-domain-foundation.md`](./generated-result-asset-domain-foundation.md) and [`generated-result-api-contracts.md`](./generated-result-api-contracts.md).
- For Feature 6 / Epic 6.4 Story 6.4.4 final completion verification and follow-on integration guidance for durable generated-result assets, read [`image-manipulation-feature-6-final-baseline.md`](./image-manipulation-feature-6-final-baseline.md).
- For Feature 7 / Epic 7.1 Story 7.1.1 canonical image-studio interaction model (ordered journey, state ownership partitions, transition/invalidation rules, and continuation/reopen semantics), read [`image-manipulation-studio-interaction-model.md`](./image-manipulation-studio-interaction-model.md).
- For Feature 7 / Epic 7.1 Story 7.1.2 studio-facing presenter contracts and composed DTO strategy (major surface contracts, centralized UI mapping selectors, standardized load/empty/error/ready/degraded states, and advanced diagnostics posture), read [`image-manipulation-studio-presenter-contracts.md`](./image-manipulation-studio-presenter-contracts.md).
- For Feature 7 / Epic 7.1 Story 7.1.3 user-facing studio UX language posture, primary flow terminology, and advanced diagnostics wording rules, read [`image-manipulation-studio-ux-language-guidelines.md`](./image-manipulation-studio-ux-language-guidelines.md).
- For Feature 7 / Epic 7.1 Story 7.1.4 full studio architecture and UX-composition posture (Feature 1-6 composition, authoritative orchestration guardrails, and state/presenter/component/API seam ownership boundaries), read [`image-manipulation-studio-feature-7-ux-composition-posture.md`](./image-manipulation-studio-feature-7-ux-composition-posture.md).
- For Feature 7 / Epic 7.4 Story 7.4.4 completion verification and follow-on Feature 8 integration guidance (flow completion baseline, architecture/UX guardrails, verification cross-references, and resilience extension points), read [`image-manipulation-studio-feature-7-ux-composition-posture.md`](./image-manipulation-studio-feature-7-ux-composition-posture.md).
- For Feature 8 / Epic 8.1 Story 8.1.2 resilience-state contracts for degraded/partial/blocked availability across execution, node, result, preview, and retrieval seams, read [`image-manipulation-resilience-state-contracts.md`](./image-manipulation-resilience-state-contracts.md).
- For Feature 8 / Epic 8.1 Story 8.1.3 retry/recovery/escalation contracts that distinguish automatic retry, manual retry, user correction, backend pending-recovery, and operator/admin escalation paths, read [`image-manipulation-retry-recovery-escalation-contracts.md`](./image-manipulation-retry-recovery-escalation-contracts.md).
- For Feature 8 / Epic 8.1 Story 8.1.4 cross-layer resilience/error-handling architecture posture (infrastructure-to-application normalization flow, API/presenter projection boundaries, and retry/recovery ownership rules), read [`image-manipulation-resilience-error-handling-architecture.md`](./image-manipulation-resilience-error-handling-architecture.md).
- For Feature 8 / Epic 8.2 Story 8.2.1 hardened ingestion/retrieval validation posture for malformed/unsupported/corrupted/stale image asset requests and normalized failure contracts, read [`image-asset-validation-retrieval-hardening.md`](./image-asset-validation-retrieval-hardening.md).
- For Feature 8 / Epic 8.2 Story 8.2.5 structured resilience diagnostics, request-to-run/result correlation, and redaction-safe observability conventions across image slice services, read [`image-manipulation-resilience-diagnostics-correlation-redaction-conventions.md`](./image-manipulation-resilience-diagnostics-correlation-redaction-conventions.md).
- For Feature 8 / Epic 8.4 Story 8.4.1 representative end-to-end resilience verification matrix coverage (invalid uploads, incompatible systems, no-eligible-node readiness, transient dispatch failure, cancelled runs, missing output collection, preview degradation, and protected retrieval unavailability), read [`image-manipulation-resilience-verification-matrix.md`](./image-manipulation-resilience-verification-matrix.md).
- For Feature 8 / Epic 8.4 Story 8.4.2 resilience-sensitive audit hardening coverage (repeated validation denials, node availability transitions, result-collection degraded/failed outcomes, protected retrieval denials, and retry/recovery governance traces), read [`run-submission-lifecycle-audit-hooks.md`](./run-submission-lifecycle-audit-hooks.md) and [`../audit-governance-contributor-guide.md`](../audit-governance-contributor-guide.md).
- For Feature 8 / Epic 8.4 Story 8.4.3 cross-feature completion guidance that unifies operational assumptions, degraded-state interpretation, diagnostics flow, and safe recovery posture across Features 1-8, read [`image-manipulation-feature-8-cross-feature-operational-guidance.md`](./image-manipulation-feature-8-cross-feature-operational-guidance.md).
- For Feature 8 / Epic 8.4 Story 8.4.4 final vertical-slice completion verification and launchable production-resilience posture baseline, read [`image-manipulation-feature-8-final-vertical-slice-completion.md`](./image-manipulation-feature-8-final-vertical-slice-completion.md).
...
- Feature 20.3.5 policy-admin observability/redaction/failure-handling baseline for structured diagnostics, correlation propagation, safe signal emission, and explicit bootstrap/read/write/admin-surface failure capture: docs/architecture/deployment-profile-policy-admin-observability-redaction-and-failure-handling.md.
- Feature 20.3.6 deployment-profile evolution seams and non-home neutrality safeguards baseline for configurable fallback profile seams, operation-driven snapshot-profile resolution, and no-placeholder profile feature posture: docs/architecture/deployment-profile-policy-evolution-seams-and-neutrality-safeguards.md.
- Feature 20.3.7 final regression hardening baseline for end-to-end policy lifecycle coverage, provenance/permission invariants, contract-stability checks, and explicit deferred-edge documentation: docs/architecture/deployment-profile-policy-regression-hardening-baseline.md.
