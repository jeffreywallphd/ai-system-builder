...
- Phase 6 authoring now has an explicit persistence/application seam split:
  ...
  - `SqliteCompat` now resolves `better-sqlite3` from module-local runtime context and then retries via explicit ancestor `node_modules/better-sqlite3` probing, so native binding lookup still targets `node_modules/better-sqlite3/build` without `process.cwd()` drift.
