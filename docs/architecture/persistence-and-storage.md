...
Workspace persistence must continue to avoid raw local path leakage in public errors/read models and must not create artifact, image, model, dataset, or other workspace resource storage directories until later Phase 6 storage/scoping work defines those boundaries. Phase 6 Prompt 4 adds an application-level workspace creation use case that writes only through these workspace ports. It validates display names, generates safe workspace IDs, persists workspace records, may persist active workspace selection only when explicitly requested, and may create a reference-only `system.foundation@1.0.0` activation record. The Phase 6 Review B cleanup wires page gating and minimal API/IPC/preload/client surfaces to these same persisted records and the create use case: UI-created workspaces are backend-resolvable workspace records, active workspace selection remains a persisted preference/read model rather than global mutable application-service state, and the System Foundation checkbox persists a reference-only `system.foundation@1.0.0` activation. It still adds no artifact/image/model/data storage directories, user library, cross-workspace reuse, resource-backed descriptor persistence, pack import/export/install UI, or collaboration behavior.
...
## Phase 6 workspace persistence stabilization
