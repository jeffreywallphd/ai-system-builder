...
- System Studio page structure now enables that shared snap contract with tenth-grid release snapping (`x=10`, `y=10`, on-release position snapping), so released sections drift toward nearest valid tenth-aligned bounds while remaining in existing selection, persistence, and layout-bound normalization seams.
