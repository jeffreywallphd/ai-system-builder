...
- Feature 4 / Epic 4.2 Story 4.2.3 authoritative run discovery/query baseline for get-by-id and history listing filters (owner/system/status/time/recent activity/completion), query-level read authorization seams, and DTO-ready history hints: `docs/architecture/run-orchestration-query-and-history-discovery.md`
- Feature 4 / Epic 4.2 Story 4.2.5 durable authoritative image-run persistence and lifecycle/status-history recording baseline (schema, adapter behavior, and migration impact): `docs/architecture/run-orchestration-image-run-persistence-and-status-history.md`
...
- Feature 2 final-baseline completion posture for authoritative studio workflow/system authoring, supported-workflow enforcement, and Feature 3/4/6 dependency handoff: `docs/architecture/image-workflow-feature-2-final-baseline.md`
...
- Feature 5 / Epic 5.1 Story 5.1.2 execution-node capability/compatibility contracts for image workflow and run routing eligibility, including hard incompatibility, soft advisory, and transient availability outcomes: `docs/architecture/execution-node-capability-compatibility-contracts.md`
- Feature 5 / Epic 5.1 Story 5.1.3 execution-node repository and management application ports for registration/query, health and capability refresh, availability controls, and routing selection hints: `docs/architecture/execution-node-repository-and-management-application-ports.md`
...
- Feature 5 / Epic 5.4 Story 5.4.4 final completion verification and follow-on integration guidance for node-based execution and backend management: `docs/architecture/image-manipulation-feature-5-final-baseline.md`
...
- Feature 6 / Epic 6.4 Story 6.4.4 final completion verification and follow-on integration guidance for durable generated-result assets: `docs/architecture/image-manipulation-feature-6-final-baseline.md`
...
- Feature 8 / Epic 8.4 Story 8.4.2 resilience-sensitive audit hardening for repeated validation denials, node availability transitions, result-collection degraded/failed outcomes, protected retrieval denials, and retry/recovery governance events: `docs/architecture/run-submission-lifecycle-audit-hooks.md`, `docs/audit-governance-contributor-guide.md`
- Feature 8 / Epic 8.4 Story 8.4.3 cross-feature operations and maintenance guidance for the full image slice (Features 1-8 assumptions, degraded-state interpretation, diagnostics flow, safe recovery paths, and extension seams): `docs/architecture/image-manipulation-feature-8-cross-feature-operational-guidance.md`
- Feature 8 / Epic 8.4 Story 8.4.4 final vertical-slice completion baseline and production-resilience handoff artifact (scope closure, explicit deferrals, extension seams, and verification anchors): `docs/architecture/image-manipulation-feature-8-final-vertical-slice-completion.md`
...
- Feature 20.3.5 policy-admin observability/redaction/failure-handling baseline for structured diagnostics, correlation propagation, safe signal publication, and explicit bootstrap/read/write/admin-surface failure capture: docs/architecture/deployment-profile-policy-admin-observability-redaction-and-failure-handling.md.
- Feature 20.3.6 deployment-profile evolution seam and non-home neutrality safeguard baseline for explicit fallback profile seams, operation-driven snapshot profile selection, and no-placeholder profile feature claims: docs/architecture/deployment-profile-policy-evolution-seams-and-neutrality-safeguards.md.
