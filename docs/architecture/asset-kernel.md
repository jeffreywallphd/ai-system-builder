...
Phase 3 Prompt 8 stabilizes that baseline for final review. Resource-backed views remain computed read models behind the application provider port and internal Asset Registry read facade; they are not `AssetInstance` records, are not durable mapping records, and are not registered/imported/finalized/localized/published by reads. Supported Phase 3 families are artifact/document, image/generated-output, dataset/model, and external-repository object views through safe descriptor/read seams only. Unsupported or unwired seams return sanitized diagnostics. Generated outputs remain explicitly unfinalized/unregistered views, external repository objects remain unimported/unregistered views, and model reads keep discovery disabled with `includeDiscovered: false`. Existing public API, IPC, preload, and Asset Library surfaces remain read-only definition list/read/version-read surfaces; internal resource-backed facade reads are available for host composition and future read-only wrappers, but Phase 3 adds no routes, channels, preload methods, UI actions, mutation workflows, scans, provider calls, network calls, runtime calls, or byte/content reads.
...
- `modules/hosts/shared/composition/composeInternalAssetRegistry.ts` composes that local kernel with the application `AssetRegistryReadFacade` for host-owned internal use. The helper accepts an optional `AssetResourceBackedViewProvider` seam, but it does not import/report seed catalog content, execute built-in seeding, scan artifacts/images/datasets/models/storage, or expose local paths in diagnostics. Desktop/server host registration now composes the helper from `storageRootDirectory` and exposes it only through host-internal getters; public API/IPC/preload/renderer/thin-client surfaces remain unchanged until a later transport/UI prompt wraps the internal facade explicitly.
