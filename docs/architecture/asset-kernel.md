...
Asset instances, compositions, registry summaries, resource-backed views, renderer Asset Library pages, thin-client Asset Library pages, routes, and navigation remain deferred. The current UI-client scope is definitions-only unless a later prompt adds matching API and IPC/preload operations. Mutation, seeding, import, finalize, register, scan, publish, and execute operations remain forbidden through Asset Registry API/IPC/preload and Asset Library UI-client surfaces.
