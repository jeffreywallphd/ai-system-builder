...
Definition records are stored by `definitionId@version`; exact `asset-definition-version` references resolve the matching version, while `asset-definition` references resolve a deterministic latest version without auto-incrementing or mutating supplied versions. Resource-backed mapping, storage-backed asset linkage, host composition wiring, transport exposure, automatic composition, and generated context remain deferred to Prompt 10 or later.
