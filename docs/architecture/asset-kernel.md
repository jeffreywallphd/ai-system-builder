...
Phase 6 adds passive workspace contracts in `modules/contracts/workspace` so later prompts can describe workspace-scoped asset/resource views with shared vocabulary. The workspace system-pack activation contract references system packs by id/version, allowing `system.foundation@1.0.0` to be represented for a workspace without copying pack manifests, asset entries, or definitions. These contracts do not add Asset Library filtering, resolver activation behavior, workspace persistence, host wiring, page gating, system-pack install/copy behavior, collaboration permissions, invites, sync, or remote auth.
