...
| Asset content encryption key (`AI_LOOM_ASSET_CONTENT_ENCRYPTION_KEY`) and key prefix (`AI_LOOM_ASSET_CONTENT_ENCRYPTION_KEY_PREFIX`) | workspace/storage-instance effective scope (provider-backed versioned key references) | Provider-backed secret runtime resolution in [VersionedServerScopedAssetContentEncryptionKeyPort.ts](../../src/infrastructure/security/encryption/VersionedServerScopedAssetContentEncryptionKeyPort.ts) composed by [ServerStorageAssetCompositionModule.ts](../../src/hosts/server/composition/ServerStorageAssetCompositionModule.ts), with fallback value sourced through [ResolveCriticalServerSecurityMaterial.ts](../../src/hosts/server/composition/ResolveCriticalServerSecurityMaterial.ts) | Active writes resolve the current secret version for `secret:server:asset-content-encryption-key` and persist versioned key references; decryption resolves exact/superseded versions by descriptor reference. If provider-backed material is unavailable, deterministic fallback remains policy-gated. Prefix defaults to `kek:asset-content`. | Durable provider-backed key versions are expected for production rollover continuity; deterministic fallback is compatibility-only and non-authoritative for production rotation governance. | Asset encryption key resolution and AES-GCM content cipher. | Host composition helper + infrastructure crypto adapter. |
...
## Story 3.3.5 Asset Content Encryption Key Rollover

- Asset-content encryption now uses versioned key references tied to secret versions from `secret:server:asset-content-encryption-key`.
- New encrypted writes automatically adopt active key material without manual record patching.
- Previously encrypted content remains readable because decryption resolves key material by stored key reference and can retrieve superseded secret versions.
- Legacy deterministic asset key references (`...:v1`) are retained through a compatibility mapping to baseline secret version `secret:server:asset-content-encryption-key:v1`.
- Full bulk re-encryption of already encrypted asset payloads is intentionally deferred; this story delivers rollover-safe read compatibility rather than immediate mass re-encryption.
