...
- Direction 4 Phase 6 inner architecture now extends the same split:
  ...
  - `SqliteCompat` now resolves `better-sqlite3` from module-local runtime context and then retries via an explicit ancestor `node_modules/better-sqlite3` path probe, avoiding `process.cwd()` drift while still targeting the native binding under `node_modules/better-sqlite3/build`.
