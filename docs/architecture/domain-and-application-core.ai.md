...
- Output validation now also enforces destination-specific readiness in that same canonical path (file-export format/delivery constraints, web-viewer title requirements, system-entry entity-name requirements, and prompt-response-chat prompt-link/scope/response-field checks) so output authoring gates are not UI-only.
