...
- Output destination definitions are now inner-layer contracts (`list/getWorkflowDraftOutputDestinationDefinitions`) with stable schema ids/defaults/formats for `file-export`, `web-viewer`, and `system-entry`, so output selector/configuration flows reuse domain truth instead of UI-local registries.
