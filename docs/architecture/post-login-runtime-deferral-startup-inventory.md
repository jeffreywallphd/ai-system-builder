...
| Workflow run summary repository | `new SqliteWorkflowRunSummaryRepository(...)`; `new ListWorkflowRunSummariesUseCase(...)`; runtime `ensureWorkflowRunHistory()` | Factory at `electron/main/DeferredDesktopFeatureRuntime.ts`; ensure in deferred runtime; usage in `electron/main/main.ts:945-967` | First workflow-run IPC use (or first studio-shell workflow-run-history method via lazy adapter) | lazy/on-demand | Shared by direct workflow-run IPC handlers and studio-shell runtime through a lazy repository adapter. |
| Studio-shell repository and workflow persistence repo | `new SqliteStudioShellRepository(...)`; `new SqliteWorkflowPersistenceRepository(...)`; runtime `ensureStudioDependencies()` | Factory methods in `electron/main/DeferredDesktopFeatureRuntime.ts`; ensure in deferred runtime | First studio/system runtime feature use | lazy/on-demand | No direct dependency on eager workflow-run-summary repository creation; workflow-run history is resolved lazily when that path is used. |
...
| Canonical registry runtime | `new SqliteAssetSystemRepository(...)` + dynamic imports + registry use cases + `new RegistryBackendApi(...)` | `createCanonicalRegistryRuntimeProvider(...).ensureCanonicalRegistryRuntime()` in `electron/main/runtime/CanonicalRegistryRuntimeProvider.ts`; invoked by on-demand composition in `electron/main/main.ts` | First canonical assets/registry IPC use | lazy/on-demand | Hard dependency on deferred runtime `ensureSystemRuntimeBackendApi()` and `ensureWorkflowPersistenceRepository()`. |
| Agent runtime repositories and backend API | `new SqliteAgentRepository(...)`; `new SqliteAgentExecutionSessionRepository(...)`; `new SqliteAssetSystemRepository(...)`; `new AgentStudioBackendApi(...)` | `createDesktopAgentRuntimeProvider(...).ensureAgentStudioBackendApi()` in `electron/main/runtime/DesktopAgentRuntimeProvider.ts`; invoked by on-demand composition in `electron/main/main.ts` | First agent IPC use | lazy/on-demand | Depends on agent runner graph and storage paths; independent from studio/system/canonical runtime chain. |
...
- `ensureWorkflowRunHistory()` is independent of `ensureStudioDependencies()` and is shared through lazy adapter injection into `ensureStudioShellBackendApi()`.
...
- Agent runtime (`DesktopAgentRuntimeProvider.ensureAgentStudioBackendApi`) is independent of canonical registry and studio/system runtime chains except shared storage roots.
