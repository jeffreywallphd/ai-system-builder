...
Phase 6 has introduced a passive `modules/contracts/workspace` contract family for workspace identity, lifecycle status, future actor/member role metadata, storage-root descriptors, system-pack activation references, workspace records, creation commands, active-selection read models, and explicit workspace request context. Phase 6 Prompt 3 adds application repository ports and local file-backed persistence adapters for workspace records/indexes, active workspace selection preferences, and workspace system-pack activation records. These foundations prepare later workspace-scoped assets and resource-backed views, but they do not implement workspace creation, page gating, Asset Library effective views, resource scoping, active workspace global application-service state, collaboration permissions, invites, sharing, sync, remote auth, API/IPC/preload/UI exposure, host wiring, or multi-user runtime behavior. Workspace system-pack activation records reference system packs such as `system.foundation@1.0.0` by pack id/version only and do not install, copy, embed, or mutate system pack definitions.
