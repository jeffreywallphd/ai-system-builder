...
- Built-in wizard actions now include `if-then`, `loop-iteration`, and `delay-wait`, with per-step type switching that clears incompatible stale config/asset references.
- Wizard step configuration is now type-specific and draft-bound: agent/assistant asset binding for asset-backed steps, condition + optional branch labels for if/then, repeat-count for loop/repeat, and duration-seconds for delay/wait.
...
- Wizard mode now includes explicit guided progression controls in `WorkflowStudioWizardModeSurface` (jump-to-section links, previous/next controls, current/upcoming/completed labels, and terminal "Finish and continue" action area) while keeping Trigger/Inputs/Steps/Outputs visible on one page.
