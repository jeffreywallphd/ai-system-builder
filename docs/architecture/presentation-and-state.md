...
- Experience-asset authoring now has a reusable UI-neutral contract seam (`ui/studio-shell/experience-assets/ExperienceAssetContracts.ts`) plus a generic mode boundary renderer (`ui/components/studio-shell/experience-assets/ExperienceAssetAuthoringBoundary.tsx`) so wizard/canvas mode switching can be shared across studios without coupling to workflow-specific route/store/domain contracts.
- Workflow Studio now resolves experience-asset modes directly inside `WorkflowStudioDraftAuthoringBoundary` and no longer depends on `ExperienceAssetAuthoringBoundary` for active runtime behavior, while preserving route-driven wizard/canvas selection and unsupported-mode fallback messaging.
- Migration audit snapshot for remaining old workflow authoring/runtime seams under the studio-shell flow:
  - **Still actively routed:** legacy standalone workflow editor route (`/workflows/:workflowId` -> `ui/pages/WorkflowEditorPage.tsx`) remains reachable for compatibility and is outside the studio-shell composite path.
  - **Still imported into active runtime paths:** `WorkflowEditorPage` remains imported in `ui/routes/AppRouter.tsx`; `WorkflowStudioDraftAuthoringBoundary` remains the active studio-shell workflow draft surface.
  - **Legacy compatibility infrastructure:** `ExperienceAssetAuthoringBoundary` remains active for Dataset/System studios but is now disconnected from Workflow Studio runtime.
  - **Already effectively dormant in workflow studio path:** workflow wizard/canvas intermediary switching through `ExperienceAssetAuthoringBoundary` is no longer part of active Workflow Studio runtime behavior.
...
- Stories 5–6 now route Workflow Studio wizard rendering through a reusable wizard asset definition seam (`ui/studio-shell/experience-assets/ConfigurableWizardSurfaceContracts.ts` + `WizardExperienceAssetDefinition` contracts) and a workflow-specific adapter (`ui/studio-shell/workflow/WorkflowWizardExperienceAdapter.tsx`) that maps workflow sections, readiness/progress, issues, page renderers, and terminal behavior into the generic configurable wizard surface.
...
- Wizard flow was narrowed to `Pages`, `Page layout`, and `Settings` in `SystemWizardExperienceAdapter`; legacy embedded data/workflow authoring steps were removed from the core System Studio flow.
