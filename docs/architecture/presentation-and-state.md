...
- Workflow Studio canvas layout now renders the React Flow canvas card first within draft authoring, with drawer/inspector/details surfaces below it, so Canvas mode reads as a primary authoring surface rather than a secondary panel.
