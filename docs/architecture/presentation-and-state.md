...
- Canvas graph projection now includes deterministic y-axis collision resolution inside each section column in `WorkflowStudioCanvasViewModel`, so overlapping section/item nodes are auto-spaced while preserving section ordering semantics.
