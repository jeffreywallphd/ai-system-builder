...
- Output configuration remains draft-bound and mode-shared: file export format/delivery/path/name, web viewer title/presentation mode, system record entity/config fields, and prompt-response chat linkage/scope/system-prompt fields persist through Wizard/Canvas mode switches using `WorkflowStudioModeStateStore`.
