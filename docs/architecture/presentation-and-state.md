...
- Wizard mode now includes explicit guided progression controls in `WorkflowStudioWizardModeSurface` (jump-to-section links, previous/next controls, current/upcoming/completed labels, and terminal handoff action area) while keeping Trigger/Inputs/Steps/Outputs visible on one page.
