...
- System Studio page structure now opts into that shared snap contract with tenth-grid release snapping (`x=10`, `y=10`, on-release position snapping), so dropped sections drift to nearest valid tenth-aligned bounds while staying in the existing selection, persistence, and normalization flow.
