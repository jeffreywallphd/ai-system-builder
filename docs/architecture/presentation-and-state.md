...
- Migration audit snapshot for workflow authoring/runtime seams under the studio-shell flow:
  ...
  - **Legacy compatibility infrastructure retired:** `ExperienceAssetAuthoringBoundary` is now disconnected from active studio runtime and marked `DEPRECATED_` for follow-up deletion.
