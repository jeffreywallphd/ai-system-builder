...
- Wizard Outputs now include an interactive output editor (`WorkflowStudioOutputSectionEditor`) over canonical `WorkflowDraft.outputs[]` with multi-output add/remove, destination-type selection (`file-export`, `web-viewer`, `system-entry`), canonical output ordering (`order`) preservation, type-specific configuration fields, and type-switch resets that clear stale destination-specific options.
