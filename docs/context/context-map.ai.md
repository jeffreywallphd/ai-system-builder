...
- Keep deterministic baseline core-pack ordering explicit: `repository-overview`, then `architecture-core`, then `context-system-foundations`; insert `runtime-and-host` before `context-system-foundations` for runtime/host-centered mappings.
