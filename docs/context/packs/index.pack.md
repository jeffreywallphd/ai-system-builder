...
- Treat the Asset Kernel as the canonical platform guardrail for assets: assets are versioned, configurable, AI-readable, machine-composable semantic units for features, systems, subsystems, and systems composed of subsystems; Prompt 4 adds detailed schema-engine-neutral configuration contracts to the existing `modules/contracts/asset` family, while detailed AI context (Prompt 5), ports/rules (Prompt 6), validation services (Prompt 7), registry ports (Prompt 8), persistence (Prompt 9), and resource-backed mapping (Prompt 10) remain deferred; do not invent parallel models for artifacts/resources/UI/workflows/tools/generated outputs/previews/AI context.
