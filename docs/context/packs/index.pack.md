...
- Phase 2C desktop Asset Registry IPC/preload exposure is definitions-only and read-only. It wraps the application read facade/read port and must not expose mutation, seeding, import/finalize/register, resource scanning, runtime/provider execution, direct persistence access, renderer UI, or thin-client UI.
