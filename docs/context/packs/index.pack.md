...
- Phase 4 Review A requires mutation command guards and safe ID-generator availability checks to run before meaningful mutation work. Finalization guard failures must not read sources, search duplicates, call finalization, or touch repositories; mutation IDs must be injected or centrally controlled, partial failures must stay sanitized/retry-safe, and public mutation exposure remains deferred.
- Phase 4 Prompt 5 adds internal application use cases for `asset.import-external-repository-object` and `asset.localize-external-repository-object`. They remain unexposed publicly, re-read the source external object view by id, require explicit network/credential/partial-completion approval plus filesystem-write approval when durable internal import/localization state may be created, call only a narrow application import/localization port for provider/network/storage effects, require safe internal backing before Asset Kernel instance registration, store metadata/references only, detect duplicates before and after port results, and return sanitized retry-safe partial failures.
