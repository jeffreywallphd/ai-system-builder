...
- Phase 2C Asset Library UI-client work is definitions-only and read-only: shared UI read models/mappers live under `modules/ui/shared/asset-library`, desktop renderer clients call preload read methods, and thin-client clients call GET-only server API routes. Phase 2C Prompt 5 adds the desktop-only top-level `Assets` page using that read-only client with advanced details collapsed by default; thin-client Asset Library UI, mutations, seeding, import/finalize/register, scans, runtime/provider execution, byte reads, and direct application/host/persistence imports remain forbidden.
