...
- Phase 2C Prompt 8 stabilizes the final read-only Asset Library baseline: server API, desktop IPC/preload, desktop renderer, and thin-client surfaces remain definition list/read/version-read only; validation is explicit/available-only; built-in seeding stays internal; resource-backed views stay computed and non-scanning; and mutation/execution/import/finalization/seeding is deferred.
...
- Phase 3 Prompt 8 stabilizes the final provider baseline: resource-backed views are computed, descriptor-only, sanitized, non-persistent read models visible through internal Asset Registry read-facade resource-backed view reads when a safe provider aggregate is composed. Public API/IPC/preload/Asset Library surfaces remain read-only definition list/read/version-read only unless a later prompt explicitly adds read-only resource-backed wrappers. Generated outputs remain unfinalized/unregistered, external repository objects remain unimported/unregistered, model discovery remains disabled, and no scans, provider/network calls, runtime/task-registry calls, byte/content reads, seeding, registration, import, finalization, localization, publishing, or mutation workflows are introduced.
