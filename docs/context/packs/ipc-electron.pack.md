...
- Phase 5 Prompt 9 does not add IPC channels. Pack/source/category discoverability should flow through existing read-only Asset Registry definition list/read/version-read payloads and shared UI mappers. Do not add pack install/import/export/activate/disable channels, override edit channels, resolver activation channels, asset editor channels, scan channels, provider/network/runtime channels, or byte/content channels.
