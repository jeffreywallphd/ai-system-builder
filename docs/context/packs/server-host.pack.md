...
Phase 2C Prompt 8 finalizes the server/thin-client Asset Library baseline as read-only: Express exposes only GET definition list/read/version-read routes, thin-client code uses the server API client and shared UI helpers, validation is explicit through `includeValidation`, built-in seeding stays internal, and no server/thin-client Asset Library code may scan resources, read bytes, call providers/runtimes, or expose mutation/import/finalization/execution controls.

Phase 3 Prompt 8 keeps server resource-backed provider wiring internal to host composition and the Asset Registry read facade. Do not add server routes, thin-client controls, automatic seeding, registration/import/finalization/localization/publishing flows, storage scans, provider/network calls, runtime/task-registry calls, or byte/content reads for resource-backed views.
