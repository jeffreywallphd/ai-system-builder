...
- Phase 5 Prompt 8 internal system pack install diagnostics must stay sanitized: no local filesystem paths, stack traces, raw persistence exceptions, tokens/secrets/auth material, provider payloads, resource bytes/base64, command lines, runtime details, or storage roots. The installer validates before save, persists only safe metadata/references, fails user/custom conflicts without overwrite or later-entry saves, and remains internal with no public API/IPC/preload/UI install surface or host startup auto-seeding.
- Phase 5 Prompt 9 Asset Library pack/source/category display must use sanitized read fields only. UI and read-facade output must omit local/cache/storage/runtime paths, tokens/secrets/auth/session/cookies, signed URLs, query credentials, raw provider payloads, stack traces, command lines, environment values, bytes/blob/base64/data URLs, prompt/negative-prompt text, workflow JSON, and resource contents. `workspace-pack` is not an override label by itself. Override/resolution metadata is informational only when safe references/summaries already exist and must not imply resolver execution or edit authority.
- Phase 5 Prompt 10 resolver diagnostics must stay sanitized and summary-only: trace, diagnostics, and conflicts may include safe refs, rule IDs, source layers, counts, and selected result metadata, but must not include local/cache/storage/runtime paths, secrets/tokens/auth values, signed URLs, raw provider payloads, stack traces, command lines, environment values, bytes/blob/base64/data URLs, prompts, workflow JSON, or resource contents. Resolver output is internal application data and must not be exposed directly through UI/API/IPC/preload. The resolver must not call persistence, storage, providers, network, runtime, filesystem, API/IPC/preload, or UI behavior.
...
- Phase 6 Prompt 2 collaboration fields are passive workspace actor/member/role placeholders only. Do not infer permission evaluation, ACLs, invite tokens/URLs, sharing, sync, sessions, remote auth, organization/team administration, or multi-user runtime behavior from these contracts.
- Phase 6 Prompt 3 workspace local persistence and Prompt 4 workspace creation diagnostics must keep public errors/read models sanitized: no raw filesystem paths, raw JSON/resource contents, stack traces, command lines, env values, secrets/tokens, provider payloads, bytes/base64, or display-name-derived paths. Active workspace selection is only a persisted preference and is saved by the create use case only when explicitly requested; it does not authorize access. System-pack activation records are references only and do not call the Phase 5 installer or install/copy trusted pack definitions. Collaboration actor/member fields remain passive placeholders only.
