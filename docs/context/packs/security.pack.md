...
- Phase 6 Prompt 3 workspace local persistence must keep public errors/read models sanitized: no raw filesystem paths, raw JSON/resource contents, stack traces, command lines, env values, secrets/tokens, provider payloads, bytes/base64, or display-name-derived paths. Active workspace selection is only a persisted preference and does not authorize access; system-pack activation records are references only and do not install/copy trusted pack definitions.
