...
Transport/UI work is deferred until after the kernel is proven through shared contracts, configuration, AI context, ports/composition, validation, registry ports, persistence, and resource-backed mapping. Prompt 10 does not add workflow execution, graph execution, UI page routing, API/IPC exposure, runtime behavior, durable resource-backed mapping registration, host wiring, or automatic composition.
...
- Durable resource-backed mapping registration, persistence/storage linkage, API/IPC/UI exposure, runtime behavior, and host wiring remain deferred.
