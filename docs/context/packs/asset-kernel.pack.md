...
- Phase 2B Prompt 7 adds `modules/hosts/shared/composition/composeInternalAssetRegistry.ts`, which composes `composeLocalAssetKernel` with the internal application Asset Registry read facade for definitions, instances, compositions, optional binding summaries, exact version-aware built-in markers/catalog matches, explicit validation, and an optional injected computed resource-backed view provider seam. Durable resource-backed mapping registration, host startup seeding execution, persisted resource-backed scans, public resource-backed views, API/IPC/preload/UI exposure, and runtime behavior remain deferred; the built-in catalog exists but is not seeded automatically at startup and host diagnostics do not report catalog counts.
