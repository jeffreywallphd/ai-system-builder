...
- Desktop/server host startup and registration do not initialize the helper solely for side effects; host execution remains deferred until an internal registry facade, seeding flow, or real private consumer needs it. Durable resource-backed mapping registration, host startup seeding execution, registry read facades, resource-backed scans/views, API/IPC/preload/UI exposure, and runtime behavior remain deferred; the built-in catalog exists but is not wired into startup.
