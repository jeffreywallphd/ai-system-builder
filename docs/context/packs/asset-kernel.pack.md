...
- Desktop/server host startup and registration do not initialize the helper solely for side effects. Phase 2B Prompt 6 adds an internal application Asset Registry read facade for definitions, instances, compositions, optional binding summaries, persisted built-in markers, explicit validation, and injected computed resource-backed views, but host-level facade composition remains deferred to Prompt 7. Durable resource-backed mapping registration, host startup seeding execution, persisted resource-backed scans, public resource-backed views, API/IPC/preload/UI exposure, and runtime behavior remain deferred; the built-in catalog exists but is not wired into startup.
