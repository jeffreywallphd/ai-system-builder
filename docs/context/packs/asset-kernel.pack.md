...
- Phase 2B Prompt 2 adds private desktop/server host composition through `modules/hosts/shared/composition/composeLocalAssetKernel.ts`; hosts pass `storageRootDirectory`, and the adapter continues to own `<storageRoot>/asset-kernel/`.
- Durable resource-backed mapping registration, host startup seeding execution, registry read facades, resource-backed scans/views, API/IPC/preload/UI exposure, and runtime behavior remain deferred; the built-in catalog exists but is not wired into startup.
