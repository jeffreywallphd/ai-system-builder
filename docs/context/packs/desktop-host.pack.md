...
- Desktop preload may expose only matching read methods (`listAssetDefinitions`, `readAssetDefinition`, and `readAssetDefinitionVersion`). Asset mutation, seeding, import, finalize, register, scan, publish, execute, renderer Asset Library UI, and thin-client UI remain deferred.
- Desktop renderer may now include a read-only Asset Library client that calls only those preload methods and maps responses through shared UI-facing read models. It must not import IPC handlers/contracts beyond the existing preload bridge shape, host composition, application services, local persistence adapters, runtime adapters, or mutation/seeding/import/finalize/scan/execute operations.
