...
- Phase 2C desktop IPC/preload may expose only read-only Asset Registry definition list/read/version-read wrappers. Desktop composition passes only the internal read facade/read port to IPC registration, not the full internal registry, repositories, mutation use cases, seed services, storage adapters, runtime adapters, provider clients, or scan seams.
