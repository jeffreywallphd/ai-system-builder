...
The registry now adds explicit task-oriented discovery paths grounded in the existing routing and context-map task model instead of introducing a parallel navigation taxonomy.

Task discoverability is now modeled with:

- `discoveryIndex.byTaskCategory`: curated record-id lists for each routing task category.
- `taskRoutingIndex`: route-hint metadata that ties each task category to canonical routing and context assets.

`taskRoutingIndex` anchors are intentionally explicit and machine-checkable:
...
The generated index view now renders a task-workflow browsing section from these fields, covering architecture review, feature decomposition, implementation work, runtime diagnostics, runtime security hardening, and documentation refactor workflows.

## Context Routing and Pack Selection Integration Status (Story 6.3.3)

Registry identifiers are now a required integration surface for routing and pack-selection assets so task guidance can resolve authoritative docs through stable keys instead of path-only references.

Integration points in this story:

- Active routing mappings in `docs/context/routing/task-to-context-routing.seed.json` now carry `relatedDocRecordIds` as required stable references.
- Routing worked examples include `relatedDocRecordIds` whenever `expectedRelatedDocOrder` includes registry-indexed docs.
- Context pack catalog entries in `docs/context/packs/context-pack-catalog.seed.json` now maintain `relatedDocRecordIds` coverage for indexed `primaryDocPath` and indexed `relatedDocPaths`.
- Foundation validation (`dev/scripts/validate-docs-foundation.cjs`) now enforces path-to-recordId alignment for indexed docs across routing and pack assets.

This keeps the registry as the single durable identity layer while allowing routing and pack selection to remain practical, explicit, and low-complexity.

## Contributor Index Usage Guidance Status (Story 6.3.4)

Contributor-facing index adoption guidance now exists as an explicit daily workflow rather than implicit tribal knowledge.

Story 6.3.4 adds:

- `docs/contributors/documentation-index-daily-usage-guide.md` for index-first daily contributor routines.
- Router links from `docs/contributors/README.md` and `docs/architecture/README.md` so contributors discover the guide in normal navigation paths.
- Workflow alignment notes in `docs/contributors/context-engineering-system-guide.md` so prompt assembly behavior stays index-first and metadata-aware.

The guide reinforces the key boundary:

- The index provides findability and candidate selection.
...
Story 6.4.2 adds lightweight cross-reference checks to keep high-value registry relationships resilient as docs move over time.

The registry validator now enforces:

- Indexed `path` and `aiPath` references must resolve to real repository files.
- When a `relatedDocs` path points to another indexed entry, the entry must also include the matching stable ID in `relatedRecordIds`.
- Existing path checks for `relatedCodePaths`, `supersedes`, and `supersededBy` continue to detect obvious broken references.

Run:

- `npm run docs:validate:registry`

Cross-reference mismatches are reported with:

- `REGISTRY_CROSS_REFERENCE_INVALID`

## Registry Maintenance and Review Expectations Status (Story 6.4.3)

Story 6.4.3 defines lightweight maintenance expectations so the registry remains trustworthy without introducing heavy manual processes.

### When Registry Entries Must Be Updated

Update affected registry entry metadata in the same change set when any of the following occur:

- Indexed doc path changes (`path` and `aiPath`), split/merge moves, or redirect stubs are introduced.
- Lifecycle or authority changes (`status`, `authoritativeness`, `supersededBy`, `supersedes`).
- Discovery signals materially change (`title`, `summary`, `keywords`, `relatedDocs`, `relatedRecordIds`, `relatedCodePaths`).
- Routing or context-pack integration changes where indexed docs are added/removed from `relatedDocRecordIds`.

For content-only wording fixes with no discovery/lifecycle impact, registry updates are optional.

### How to Add New Documentation to the Registry

Use this practical add-flow for new high-value docs:

1. Confirm the doc belongs to required/selective coverage in `coveragePolicy` and `documentation-index-coverage-rules.md`.
2. Start from a template in `docs/context/templates/documentation-registry-entry*.json`.
3. Add one `entries` record with a stable `recordId` (`doc-...`, lowercase kebab-case) and required metadata.
4. Update `discoveryIndex` membership (`byDocType`, `byStatus`, `byDomain`, `byAuthoritativeness`, and `byTaskCategory` if relevant).
5. Add/update stable links in routing or pack assets via `relatedDocRecordIds` where the doc participates in workflows.
6. Run `npm run docs:validate:registry`, and if discovery grouping changed, run `npm run docs:generate:index-view` and `npm run docs:validate:foundation`.

### Who Should Care About Stale Entries

Primary ownership remains with `team:developer-experience`, but stale detection is shared:

- Documentation authors and story implementers update registry metadata when they change indexed docs.
- Reviewers treat missing or stale registry updates as merge blockers for docs that affect discovery, routing, or lifecycle state.
- Context/routing maintainers update `relatedDocRecordIds` when task mappings or pack associations change.

This keeps upkeep aligned to normal PR review, rather than requiring a separate registry-maintenance cycle.

### Handling Superseded, Deprecated, and Historical Transitions

When a doc stops being current authority:

1. Keep the existing `recordId`; do not create a replacement ID for the same path lineage.
2. Update `status` to `superseded`, `deprecated`, or `archived` as appropriate, and set `authoritativeness: historical` when it is no longer implementation authority.
3. Populate `supersededBy` and/or `supersedes` and keep redirect targets resolvable.
4. Ensure status indexes move the record into the correct non-active bucket and keep canonical replacements indexed as `active` where applicable.
5. Preserve discoverability for traceability, but route implementation work to active canonical destinations.
