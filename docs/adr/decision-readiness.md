...
| Multi-user collaboration, organization tenancy, and shared libraries | proposed | ADR-0029 recommends pooled organization tenancy, OIDC-backed managed identity, application authorization, PostgreSQL row security, and tenant-aligned storage; workspace, user-library, security, and persistence docs still defer behavior until acceptance | Review and explicitly accept, revise, or reject ADR-0029. Do not infer organization ownership or add public behavior or a durable tenant schema while it remains proposed. |
