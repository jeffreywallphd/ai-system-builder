...
- Ongoing monitoring loop for false positives, rule drift, and contributor friction: `docs/contributors/documentation-quality-monitoring-and-feedback-guide.md`.
- Current rollout boundaries, known limits, and future-enhancement envelope: `docs/contributors/documentation-quality-rollout-boundaries-and-follow-on-opportunities-guide.md`.
...
- Exceptions are allowed only for legitimate non-standard cases:
  - external contract mismatch,
  - security/legal constraints,
  - temporary migration constraints.
- Exceptions are not allowed for convenience, deadline pressure, or broad legacy bypasses.
- Keep exception scope narrow and path-specific; do not request directory-wide waivers.
- Record required fields (`rule_ids`, `paths`, `reason`, `owner`, `expiry_or_review_date`, `mitigation`) in the PR or linked issue.
- Use `docs/contributors/documentation-quality-exceptions-and-escape-hatches-guide.md` for workflow and anti-abuse boundaries.
