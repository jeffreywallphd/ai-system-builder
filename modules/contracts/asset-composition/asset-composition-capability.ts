...
export function normalizeAssetCompositionCapability(v:AssetCompositionCapability):AssetCompositionCapability{ if(!isSafeText(v.key)|| (v.label && !isSafeText(v.label))) throw new Error("Asset composition capability fields are invalid."); if(v.safeMetadata){ for(const [k,val] of Object.entries(v.safeMetadata)){ if(!isSafeText(k)|| (typeof val==="string" && UNSAFE.test(val))) throw new Error("Asset composition capability metadata is invalid."); }} return {...v, kind: normalizeAssetCompositionCapabilityKind(v.kind)};}
