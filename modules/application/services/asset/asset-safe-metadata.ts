...
const FORBIDDEN_ASSET_METADATA_KEY_PATTERN =
  /(token|secret|password|credential|authorization|auth|localpath|filesystempath|filepath|path|cache|bytes|blob|contentbase64|base64|raw|payload|command|stack|env)/i;
const LOCAL_FILESYSTEM_PATH_VALUE_PATTERN = /(^~\/|^\.\.?\/|^\/(?:tmp|var|home|users|etc|private|opt|usr|mnt|volumes)(?:\/|$)|^[a-z]:[\\/]|\\(?:Users|Temp)\\|\/(?:tmp|temp)\/)/i;
const AUTH_BEARING_VALUE_PATTERN = /(bearer\s+[a-z0-9._~+/=-]+|(?:api[_-]?key|api\s+key|apikey)\s*[=:]|(?:token|password|secret)\s*[=:]|authorization\s*:)/i;
