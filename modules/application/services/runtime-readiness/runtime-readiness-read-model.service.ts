...
export class WorkspaceRuntimeReadinessReadModelService {
  ...
  public async listRuntimeReadinessSummaries(request: { targetWorkspaceId: string; limit?: number; cursor?: string; archived?: boolean }) {
    const { records, nextCursor } = await this.d.bindingRepository.listRuntimeReadinessBindingRecords({ targetWorkspaceId: request.targetWorkspaceId as never, limit: request.limit, cursor: request.cursor, archived: request.archived });
  ...
  public async readRuntimeReadinessDetail(request: { targetWorkspaceId: string; readinessBindingId: string }) {
    const record = await this.d.bindingRepository.readRuntimeReadinessBindingRecord(request.targetWorkspaceId as never, request.readinessBindingId as never);
  ...
  public async listRuntimeReadinessForCompositionPlan(request: { targetWorkspaceId: string; compositionPlanId: string }) { const records = await this.d.bindingRepository.listRuntimeReadinessBindingRecordsByCompositionPlanId(request.targetWorkspaceId as never, request.compositionPlanId as never); return records.map((r) => this.toSummary(r)); }
  ...
  public async summarizeWorkspaceRuntimeInventory(request: { targetWorkspaceId: string }) {
    const { records } = await this.d.inventoryRepository.listRuntimeInventoryRecords({ targetWorkspaceId: request.targetWorkspaceId as never });
