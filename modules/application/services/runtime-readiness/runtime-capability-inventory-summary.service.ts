...
import { createWorkspaceId } from "../../../contracts/runtime-readiness";
...
export class RuntimeCapabilityInventorySummaryService {
  ...
  public async summarizeRuntimeCapabilities(request: { targetWorkspaceId: string }): Promise<RuntimeCapabilityInventoryOperationResult<WorkspaceRuntimeCapabilitySummary>> {
    ...
    return { status: "success", diagnostics: [], value: { inventorySources: result.records.length, providerCandidates: providerSummaries.length, capabilities: capabilitySummaries.length, capabilitiesByKind, providerKinds, availabilityStatuses, requiresConfigurationCount: capabilitySummaries.filter((item) => item.configurationStatus === "requires-configuration").length + providerSummaries.filter((item) => item.configurationStatus === "requires-configuration").length, requiresPermissionCount: capabilitySummaries.filter((item) => item.availabilityStatus === "requires-permission").length + providerSummaries.filter((item) => item.availabilityStatus === "requires-permission").length, blockedCount: result.records.filter((record) => record.inventoryStatus === "blocked").length, staleCount: result.records.filter((record) => record.inventoryStatus === "stale").length, diagnosticCount: result.records.reduce((sum, record) => sum + record.diagnostics.length, 0), lastCheckedAt: checked.at(-1), capabilitySummaries, providerSummaries, sourceRecords: result.records } };
  }
}
