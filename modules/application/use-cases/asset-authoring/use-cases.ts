...
export class CreateWorkspaceAuthoredAssetUseCase {
  ...
  public async execute(command: CreateWorkspaceAuthoredAssetCommand): Promise<CreateWorkspaceAuthoredAssetResult> { try {
    ...
    const authored = normalizeAuthoredAssetRecord({ authoredAssetId, workspaceId: c.workspaceId, assetReference: { kind: "asset-instance", id: authoredAssetId, version: "1.0.0", label: c.initialEditableValues["display-name"] as string | undefined }, currentRevisionId: revisionId, status: "draft", editableValues: normalizeSafeAssetEditableFieldPatch(c.initialEditableValues), provenance: { kind: "authored-from-scratch", targetWorkspaceId: c.workspaceId, operationAt: at }, createdAt: at, updatedAt: at });
