...
export class SubmitConversationTurnUseCase {
  ...
  public async execute(command: SubmitConversationTurnCommand): Promise<SubmitConversationTurnResult> {
    ...
    const session = await this.d.sessionRepository.getConversationSessionById(workspaceId, command.conversationSessionId as any);
    if (!session) {
    ...
    if (!runtimeReference || runtimeReference.status !== 'supported') {
    ...
    await this.d.turnRepository.saveConversationTurn({ id: turnId as any, workspaceId, conversationSessionId: session.id, sourceExecutionPlanId: session.sourceExecutionPlanId, userMessageId: messageId as any, status: 'submitted', sequence: session.turnIds.length + 1, blockers: [], diagnostics: [], provenance: [{ at: now, kind: 'conversation-turn-created', actorId: 'application' }], createdAt: now, updatedAt: now });
    await this.d.executionRunRepository.saveExecutionRun({ id: runId as any, workspaceId, sourceExecutionPlanId: session.sourceExecutionPlanId, sourceCompositionPlanId: session.sourceCompositionPlanId, sourceRuntimeReadinessBindingId: session.sourceRuntimeReadinessBindingId, sourceContextKind: 'conversation-turn', sourceContextId: turnId, status: 'queued', attemptIds: [attemptId as any], eventIds: [], resultIds: [], blockers: [], diagnostics: [], provenance: [{ at: now, kind: 'execution-run-start-requested', actorId: 'application' }], createdAt: now, updatedAt: now });
    await this.d.executionAttemptRepository.saveExecutionAttempt({ id: attemptId as any, workspaceId, executionRunId: runId as any, attemptNumber: 1, runtimeReferenceId: session.runtimeReferenceId ?? 'runtime-reference-missing', status: 'pending', blockers: [], diagnostics: [], provenance: [{ at: now, kind: 'execution-run-attempt-created', actorId: 'application' }], createdAt: now, updatedAt: now });
    ...
    await this.d.turnRepository.updateConversationTurn({ ...(await this.d.turnRepository.getConversationTurnById(workspaceId, turnId as any))!, executionRunId: runId as any, status: 'generating', updatedAt: now });
    await this.d.executionRunRepository.updateExecutionRun({ ...(await this.d.executionRunRepository.getExecutionRunById(workspaceId, runId as any))!, status: 'running', startedAt: now, updatedAt: now });
    await this.d.executionAttemptRepository.updateExecutionAttempt({ ...(await this.d.executionAttemptRepository.getExecutionAttemptById(workspaceId, attemptId as any))!, status: 'started', startedAt: now, updatedAt: now });
    ...
    if (orchestration.status !== 'completed') {
      await this.d.turnRepository.updateConversationTurn({ ...(await this.d.turnRepository.getConversationTurnById(workspaceId, turnId as any))!, status: 'failed', updatedAt: now, completedAt: now });
      ...
      await this.d.executionAttemptRepository.updateExecutionAttempt({ ...(await this.d.executionAttemptRepository.getExecutionAttemptById(workspaceId, attemptId as any))!, status: orchestration.status === 'timed-out' ? 'timed-out' : 'failed', updatedAt: now, completedAt: now, failureClassification });
      await this.d.executionRunRepository.updateExecutionRun({ ...(await this.d.executionRunRepository.getExecutionRunById(workspaceId, runId as any))!, status: 'failed', updatedAt: now, completedAt: now });
      const fail = this.fail('blocked', 'conversation-turn-invocation-failed', 'Conversation invocation failed.');
      await this.d.operationRepository.saveConversationOperation({ workspaceId: command.workspaceId, conversationSessionId: command.conversationSessionId, operationId: command.operationId, status: 'failed', result: fail, createdAt: now, updatedAt: now });
      return fail;
    }
    const resultId = this.d.nextId(); const assistantId = this.d.nextId(); const assistantResponseText = orchestration.assistantResponseText ?? '';
    let recordingFailureCode: 'response-recording-failed' | 'result-recording-failed' | undefined;
    try {
      await this.d.assistantResponseRepository.saveAssistantResponse({ id: assistantId as any, workspaceId, conversationSessionId: session.id, conversationTurnId: turnId as any, executionRunId: runId as any, executionResultId: resultId as any, role: 'assistant', contentKind: 'plain-text', text: assistantResponseText, status: 'completed', createdAt: now, completedAt: now });
    } catch { recordingFailureCode = 'response-recording-failed'; }
    if (!recordingFailureCode) {
      try {
        await this.d.executionResultRepository.saveExecutionResult({ id: resultId as any, workspaceId, executionRunId: runId as any, executionAttemptId: attemptId as any, kind: 'assistant-response', status: 'completed', assistantResponseId: assistantId, conversationTurnId: turnId, summary: 'Assistant response available.', createdAt: now, completedAt: now, blockers: [], diagnostics: [], provenance: [{ at: now, kind: 'execution-run-result-recorded', actorId: 'application' }] });
      } catch { recordingFailureCode = 'result-recording-failed'; }
    }
    if (recordingFailureCode) {
      await this.d.turnRepository.updateConversationTurn({ ...(await this.d.turnRepository.getConversationTurnById(workspaceId, turnId as any))!, status: 'failed', updatedAt: now, completedAt: now });
      await this.d.executionAttemptRepository.updateExecutionAttempt({ ...(await this.d.executionAttemptRepository.getExecutionAttemptById(workspaceId, attemptId as any))!, status: 'failed', updatedAt: now, completedAt: now, failureClassification: recordingFailureCode });
      await this.d.executionRunRepository.updateExecutionRun({ ...(await this.d.executionRunRepository.getExecutionRunById(workspaceId, runId as any))!, status: 'failed', updatedAt: now, completedAt: now });
    ...
    await this.d.turnRepository.updateConversationTurn({ ...(await this.d.turnRepository.getConversationTurnById(workspaceId, turnId as any))!, assistantResponseId: assistantId as any, status: 'succeeded', updatedAt: now, completedAt: now });
    await this.d.executionAttemptRepository.updateExecutionAttempt({ ...(await this.d.executionAttemptRepository.getExecutionAttemptById(workspaceId, attemptId as any))!, status: 'succeeded', resultId: resultId as any, updatedAt: now, completedAt: now });
    await this.d.executionRunRepository.updateExecutionRun({ ...(await this.d.executionRunRepository.getExecutionRunById(workspaceId, runId as any))!, status: 'succeeded', resultIds: [resultId as any], updatedAt: now, completedAt: now });
    const ok: SubmitConversationTurnResult = { kind: 'success', value: { conversationTurnId: turnId, executionRunId: runId, assistantResponseId: assistantId, assistantResponseText, status: 'succeeded' } };
