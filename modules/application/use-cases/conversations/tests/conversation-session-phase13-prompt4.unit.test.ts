...
test('eligibility returns deferred turn invocation truthfully', async () => {
  const uc = new ValidateConversationSessionEligibilityUseCase({ executionPlanRepository:{getExecutionPlanById:async()=>basePlan} as any, runtimeReadinessRepository:{readRuntimeReadinessBindingRecord:async()=>baseReadiness} as any, eligibilityService:new ConversationalExecutionPlanEligibilityService(), sourceVerificationService:new ConversationalSourceSystemVerificationService() });
  const result = await uc.execute({ workspaceId, sourceExecutionPlanId: 'execution.plan.1' });
  assert.equal(result.status, 'eligible');
  assert.equal(result.turnInvocation, 'deferred');
});
...
test('turn eligibility remains deferred without runtime adapter', async () => {
  ...
  assert.equal(result.invocation, 'deferred');
});
