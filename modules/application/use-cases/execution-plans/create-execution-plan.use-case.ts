...
export class CreateExecutionPlanUseCase {
  ...
  public async execute(command: CreateExecutionPlanCommand): Promise<CreateExecutionPlanResult> {
    ...
    try {
      ...
      const providerPlan = this.d.providerPlanningService.plan({ planId, sourceRuntimeReadinessBindingId: c.runtimeReadinessBindingId, readiness, steps: stepPlan.steps, createAdapterReferenceId: this.d.nextExecutionAdapterReferenceId });
