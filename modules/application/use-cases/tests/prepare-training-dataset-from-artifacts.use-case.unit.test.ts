...
const datasetPreparation = { startPrepareTrainingDataset: testDouble.fn(), readPrepareTrainingDatasetStatus: testDouble.fn(), cancelPrepareTrainingDataset: testDouble.fn() };
...
describe("PrepareTrainingDatasetFromArtifactsUseCase", () => {
  it("uses runtime task registry for start/read and materializes", async () => {
    ...
    const useCase = new PrepareTrainingDatasetFromArtifactsUseCase({ datasetPreparation, runtimeTaskRegistry: createRegistry({ startTask: runtimeStart, getTaskStatus: runtimeStatus }), storageBindings:{ readArtifactStorageBindings:testDouble.fn(async()=>({ok:true,value:{bindings:[]}})), upsertArtifactStorageBinding:testDouble.fn(), deleteArtifactStorageBindings:testDouble.fn() }, storage:{ retrieveArtifact:testDouble.fn(async()=>({ok:true,value:{descriptor:{key:"a1",mediaType:"text/markdown",metadata:{}},content:new TextEncoder().encode("hi")}})), storeArtifact:testDouble.fn(async(request:any)=>({ok:true,value:request.descriptor})), hasArtifact:testDouble.fn(), deleteArtifact:testDouble.fn() }, taskPowerLifecycle: lifecycle });
    ...
    expect(datasetPreparation.startPrepareTrainingDataset).not.toHaveBeenCalled();
    expect(datasetPreparation.readPrepareTrainingDatasetStatus).not.toHaveBeenCalled();
