...
export class RefreshOverrideEffectiveProjectionUseCase {
  ...
  async execute(command:RefreshEffectiveAssetProjectionCommand):Promise<RefreshEffectiveAssetProjectionResult>{
    ...
    const status = !validation.ok ? (validation.code==="effective-projection-materialization-unavailable"?"blocked":"conflicted") : mapped.blocked?"blocked":statusModel.status;
    const projection = {...existing,status,policy:status==="ready"?"safe-fields-only":"blocked",projectedFields:status==="ready"?mapped.projectedFields:{},diagnostics:validation.ok?mapped.diagnostics:defaultEffectiveAssetProjectionValidationService.asDiagnostics(validation.code).diagnostics,blockers:validation.ok?mapped.blockers:defaultEffectiveAssetProjectionValidationService.asDiagnostics(validation.code).blockers,provenance:createProjectionProvenance(c.targetWorkspaceId,existing.source,now),updatedAt:now,materializedAt:now};
