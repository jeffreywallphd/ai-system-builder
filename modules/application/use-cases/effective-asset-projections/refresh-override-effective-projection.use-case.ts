...
export class RefreshOverrideEffectiveProjectionUseCase {
  ...
  async execute(command:RefreshEffectiveAssetProjectionCommand):Promise<RefreshEffectiveAssetProjectionResult>{
    ...
    if(!o){const upd={...existing,status:"source-missing" as const,policy:"blocked" as const,projectedFields:{},diagnostics:[defaultEffectiveAssetProjectionDiagnosticsService.createDiagnostic("effective-projection-source-missing")],blockers:[defaultEffectiveAssetProjectionDiagnosticsService.createBlocker("effective-projection-source-missing")],updatedAt:now,materializedAt:now}; await this.d.projectionRepository.updateEffectiveAssetProjectionRecord(upd); return {status:"success",value:upd};}
    ...
    if(!target){const upd={...existing,status:"source-missing" as const,policy:"blocked" as const,projectedFields:{},diagnostics:[defaultEffectiveAssetProjectionDiagnosticsService.createDiagnostic("effective-projection-source-missing")],blockers:[defaultEffectiveAssetProjectionDiagnosticsService.createBlocker("effective-projection-source-missing")],updatedAt:now,materializedAt:now}; await this.d.projectionRepository.updateEffectiveAssetProjectionRecord(upd); return {status:"success",value:upd};}
    ...
    const upd={...existing,status,policy:(status==="ready"&&!mapped.blocked)?"safe-fields-only":"blocked",projectedFields:(status==="ready"&&!mapped.blocked)?mapped.projectedFields:{},diagnostics:statusModel.diagnosticCode?[defaultEffectiveAssetProjectionDiagnosticsService.createDiagnostic(statusModel.diagnosticCode)]:mapped.diagnostics,blockers:statusModel.diagnosticCode?[defaultEffectiveAssetProjectionDiagnosticsService.createBlocker(statusModel.diagnosticCode)]:mapped.blockers,provenance:createProjectionProvenance(c.targetWorkspaceId,existing.source,now),updatedAt:now,materializedAt:now};
