...
function registerRoute(app: ModelManagementExpressRoutePort, logger: ModelRouteLogger | undefined, path: string, operation: `${Lowercase<string>}.${Lowercase<string>}`, execute: (body: unknown) => Promise<unknown>) { app.post(path, async (request, response) => { const context = contextFrom(request); const requestSummary = summarizeBody(request.body); logger?.info("model-management.route.request", { operation, path, ...requestSummary }); try { const value = await execute(request.body); const apiResponse = createApiSuccessResponse(operation, value, context); logger?.info("model-management.route.success", { operation, path, resultKeys: isObjectRecord(value) ? Object.keys(value) : [] }); response.status(statusCode(apiResponse)).json(apiResponse); } catch (error) { const message = error instanceof Error ? error.message : "Unexpected error."; logger?.warn("model-management.route.failure", { operation, path, ...requestSummary, code: mapFailureCode(error), message }); const apiResponse = createApiFailureResponse(createApiError(operation, mapFailureCode(error), message, context), context); response.status(statusCode(apiResponse)).json(apiResponse); } }); }
