...
export class LocalEffectiveAssetProjectionRecordStore { private readonly dir: string; private readonly now: () => string; private readonly rootDir:string; private readonly documents?:StructuredDocumentStore; constructor(o: { rootDir: string; now?: () => string; documents?:StructuredDocumentStore }) { this.dir = join(o.rootDir, "effective-asset-projections"); this.rootDir=o.rootDir; this.documents=o.documents; this.now = o.now ?? (() => new Date().toISOString()); }
...
private async readJson<T>(f: string, fallback: T): Promise<T> { if(this.documents)return (await readDocumentRecord({rootDirectory:this.rootDir,documents:this.documents},`effective-asset-projections/${f}`,fallback)).value; try { return JSON.parse(await readFile(join(this.dir, f), "utf8")) as T; } catch (e) { if ((e as NodeJS.ErrnoException).code === "ENOENT") return cloneJson(fallback); if (e instanceof SyntaxError) throw new LocalEffectiveAssetProjectionRecordStoreError("Effective asset projection local store contains malformed JSON."); throw e; } }
private async writeJson(f: string, v: unknown) { if(this.documents){await writeDocumentRecord({rootDirectory:this.rootDir,documents:this.documents},`effective-asset-projections/${f}`,v);return;} const p = join(this.dir, f); const t = `${p}.${process.pid}.${randomUUID()}.tmp`; await writeFile(t, `${JSON.stringify(v, null, 2)}\n`, "utf8"); await rename(t, p).catch(async (e) => { await unlink(t).catch(() => undefined); throw e; }); }
}
