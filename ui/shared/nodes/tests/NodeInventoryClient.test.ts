...
describe("HttpNodeInventoryClient", () => {
  it("calls node inventory endpoints with query filters and bearer auth", async () => {
    ...
    await client.revokeNodeTrust({
      nodeId: "node:compute:1",
      reason: "operator-action",
      note: "Revoked by admin review.",
    }, "token-3");

    expect(requests.map((entry) => entry.method)).toEqual(["GET", "GET", "POST"]);
    expect(requests.map((entry) => entry.url)).toEqual([
      ...
      "http://127.0.0.1:8788/api/v1/nodes/node%3Acompute%3A1/revoke",
    ]);
    ...
    expect(requests[2]?.body).toBe("{\"reason\":\"operator-action\",\"note\":\"Revoked by admin review.\"}");
  });
});
