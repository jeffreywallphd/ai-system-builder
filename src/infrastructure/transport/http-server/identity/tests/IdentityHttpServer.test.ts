...
describe("IdentityHttpServer", () => {
  ...
  it("creates a default workspace during session-context bootstrap when the authenticated actor has none", async () => {
    const logger = new CapturingLogger();
    ...
    const createWorkspaceSlugs: string[] = [];
    const workspaceAdministrationBackendApi = {
      listWorkspaces: async () => Object.freeze({
        ok: true,
        data: Object.freeze({
      ...
      createWorkspace: async (request: { readonly slug: string; readonly displayName: string }) => {
        createWorkspaceSlugs.push(request.slug);
    ...
    const { baseUrl } = await startServer(logger, {}, {
      workspaceAdministrationBackendApi: workspaceAdministrationBackendApi as never,
    });
    ...
    const contextBody = await contextResponse.json();
    expect(contextBody.ok).toBe(true);
    expect(createWorkspaceSlugs.length).toBe(1);
