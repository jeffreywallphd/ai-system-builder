...
import {
  ...
  type RetireSecretVersionRequest,
  type RetireSecretVersionResult,
  type SecretServiceResult,
} from "./SecretManagementServiceContracts";

export interface RetireSecretVersionUseCaseDependencies {
...
export class RetireSecretVersionUseCase {
  private readonly now: () => Date;
  private readonly observabilityPort: ISecretObservabilityPort;

  public constructor(private readonly dependencies: RetireSecretVersionUseCaseDependencies) {
    this.now = dependencies.now ?? (() => new Date());
    this.observabilityPort = dependencies.secretObservabilityPort ?? new NoOpSecretObservabilityPort();
  }

  public async execute(request: RetireSecretVersionRequest): Promise<SecretServiceResult<RetireSecretVersionResult>> {
    ...
    const retiredAt = normalizeTimestamp(request.retiredAt, this.now);
    if (!retiredAt) {
      await this.emitOperation("rejected", {
        occurredAt: this.now().toISOString(),
        actorId,
        secretId,
        details: Object.freeze({
          reason: "invalid-retiredAt",
        }),
      });
      return invalidRequest("retiredAt must be a valid timestamp when provided.");
    }
    ...
    try {
      ...
      const retired = retireSecretVersionRecord({
        record,
        versionId,
        retiredBy: actorId,
        retiredAt,
      });
      const persisted = await this.dependencies.secretRecordRepository.saveSecret(retired, {
        operationKey,
        actorId,
        occurredAt: retiredAt,
      });
      ...
      return {
        ...
        error: Object.freeze({
          ...
          message: "Secret version retirement failed due to an internal security error.",
        }),
      };
    }
  }
...
function invalidRequest(message: string): SecretServiceResult<RetireSecretVersionResult> {
  return {
    ok: false,
    error: Object.freeze({
...
function notFound(secretId: string): SecretServiceResult<RetireSecretVersionResult> {
  return {
    ok: false,
    error: Object.freeze({
