...
import {
  ...
  type RevokeSecretVersionRequest,
  type RevokeSecretVersionResult,
  type SecretServiceResult,
} from "./SecretManagementServiceContracts";

export interface RevokeSecretVersionUseCaseDependencies {
...
export class RevokeSecretVersionUseCase {
  private readonly now: () => Date;
  private readonly observabilityPort: ISecretObservabilityPort;

  public constructor(private readonly dependencies: RevokeSecretVersionUseCaseDependencies) {
    this.now = dependencies.now ?? (() => new Date());
    this.observabilityPort = dependencies.secretObservabilityPort ?? new NoOpSecretObservabilityPort();
  }

  public async execute(request: RevokeSecretVersionRequest): Promise<SecretServiceResult<RevokeSecretVersionResult>> {
    ...
    const versionId = normalizeRequired(request.versionId);
    if (!versionId) {
      await this.emitOperation("rejected", {
        occurredAt: this.now().toISOString(),
        actorId,
        secretId,
        details: Object.freeze({
          reason: "missing-versionId",
        }),
      });
      return invalidRequest("versionId is required.");
    }

    const revokedAt = normalizeTimestamp(request.revokedAt, this.now);
    if (!revokedAt) {
      await this.emitOperation("rejected", {
        occurredAt: this.now().toISOString(),
        actorId,
        secretId,
        details: Object.freeze({
          reason: "invalid-revokedAt",
        }),
      });
      return invalidRequest("revokedAt must be a valid timestamp when provided.");
    }

    const reason = normalizeOptional(request.reason);
    ...
    try {
      ...
      const revoked = revokeSecretVersionRecord({
        record,
        versionId,
        revokedBy: actorId,
        revokedAt,
      });
      const persisted = await this.dependencies.secretRecordRepository.saveSecret(revoked, {
        operationKey,
        actorId,
        occurredAt: revokedAt,
      });
      ...
      return {
        ...
        error: Object.freeze({
          ...
          message: "Secret version revocation failed due to an internal security error.",
        }),
      };
    }
  }
...
function invalidRequest(message: string): SecretServiceResult<RevokeSecretVersionResult> {
  return {
    ok: false,
    error: Object.freeze({
...
function notFound(secretId: string): SecretServiceResult<RevokeSecretVersionResult> {
  return {
    ok: false,
    error: Object.freeze({
