...
export interface PublishConfigProfileDraftCommand {
  readonly studioId?: string;
  readonly sessionId: string;
...
export class ConfigProfileStudioApplicationService {
  constructor(
  ...
  public async ensureStudioInitialized(
    studioId: string = ConfigProfileStudioIdentity.defaultStudioId,
    studioName: string = ConfigProfileStudioIdentity.defaultStudioName,
  ): Promise<EnsureConfigProfileStudioResult> {
  ...
  public async createConfigProfileDraft(command: CreateConfigProfileDraftCommand): Promise<AssetDraftResult> {
    const studioId = command.studioId?.trim() || ConfigProfileStudioIdentity.defaultStudioId;
  ...
  public async publishConfigProfileDraft(command: PublishConfigProfileDraftCommand): Promise<AssetVersionResult> {
    const studioId = command.studioId?.trim() || ConfigProfileStudioIdentity.defaultStudioId;
