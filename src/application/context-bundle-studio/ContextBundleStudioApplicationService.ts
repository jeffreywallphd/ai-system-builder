...
} from "@domain/context-bundle-studio/ContextBundleStudioDomain";

export interface EnsureContextBundleStudioResult {
  readonly initialized: boolean;
  readonly studio: StudioInitializationResult["studio"];
  readonly session: StudioInitializationResult["activeSession"];
}

export interface CreateContextBundleDraftCommand {
  ...
  readonly behaviorKind?: "none" | "deterministic";
  readonly dependencies?: ReadonlyArray<AssetDraftDependencyReference>;
}

export interface PublishContextBundleDraftCommand {
  readonly studioId?: string;
  readonly sessionId: string;
...
export class ContextBundleStudioApplicationService {
  constructor(
  ...
  public async ensureStudioInitialized(
    studioId: string = ContextBundleStudioIdentity.defaultStudioId,
    studioName: string = ContextBundleStudioIdentity.defaultStudioName,
  ): Promise<EnsureContextBundleStudioResult> {
  ...
  public async createContextBundleDraft(command: CreateContextBundleDraftCommand): Promise<AssetDraftResult> {
    const studioId = command.studioId?.trim() || ContextBundleStudioIdentity.defaultStudioId;
    const taxonomy = createContextBundleStudioTaxonomy(command.behaviorKind);
    ...
    return this.studioShellService.createAssetDraft({
      ...
      metadata: createContextBundleAssetMetadata({
        title: command.title,
  ...
  private async assertPublishConsistency(studioId: string, draftId: string): Promise<void> {
    ...
    assertCompositeStudioDraftPublishConsistency({
      ...
      expectation: {
        ...
        allowedBehaviorKinds: ["none", "deterministic"],
      },
      contractResolver: this.contractResolver,
    });
  }

  public async publishContextBundleDraft(command: PublishContextBundleDraftCommand): Promise<AssetVersionResult> {
    const studioId = command.studioId?.trim() || ContextBundleStudioIdentity.defaultStudioId;
