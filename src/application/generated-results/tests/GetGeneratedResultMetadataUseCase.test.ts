...
import { GetGeneratedResultMetadataUseCase } from "../use-cases/GetGeneratedResultMetadataUseCase";
...
function buildFixture() {
  ...
  const useCase = new GetGeneratedResultMetadataUseCase({
...
describe("GetGeneratedResultMetadataUseCase", () => {
  it("returns metadata detail with preview and lineage summaries for authorized callers", async () => {
    const fixture = buildFixture();
    const result = buildResultRecord();
    fixture.repository.records.set(result.resultAssetId, result);
    fixture.repository.previews.set(result.resultAssetId, Object.freeze([Object.freeze({
      derivativeId: "gr-preview-001",
      resultAssetId: result.resultAssetId,
      resultLogicalAssetVersionId: result.logicalAssetVersionId,
      ...
      accessHandle: "preview-access://gr-preview-001",
      mediaType: "image/png",
      width: 512,
      height: 512,
      byteSize: 4096,
      generatedAt: "2026-04-08T12:02:05.000Z",
      tenancy: createWorkspaceTenancyMetadata("workspace-alpha"),
      createdAt: "2026-04-08T12:02:05.000Z",
      createdBy: "user-owner",
      lastModifiedAt: "2026-04-08T12:02:05.000Z",
      lastModifiedBy: "user-owner",
      revision: 1,
      schemaVersion: 1,
    })]));
    fixture.repository.lineages.set(result.resultAssetId, Object.freeze({
      resultAssetId: result.resultAssetId,
      runId: result.runId,
    ...
    expect(outcome.value.result.resultAssetId).toBe(result.resultAssetId);
    expect(outcome.value.result.preview.state).toBe("preview-available");
  ...
  it("returns safe not-found for private result owners when actor is not owner/admin", async () => {
    const fixture = buildFixture();
    fixture.repository.records.set("gr-result-001", buildResultRecord({
      ownerUserId: "user-owner",
      visibility: "private",
    }));
