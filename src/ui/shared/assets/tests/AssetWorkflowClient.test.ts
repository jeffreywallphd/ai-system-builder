...
describe("HttpAssetWorkflowClient", () => {
  it("calls list/detail/download/preview/upload endpoints with bearer auth", async () => {
    ...
    (globalThis as typeof globalThis & {
      ...
      if (url.includes("/upload-sessions/") && url.endsWith("/content?workspaceId=workspace-1")) {
        return new Response(JSON.stringify({
          ok: true,
          data: {
            asset: { assetId: "asset-1" },
            uploadSessionId: "session-1",
            finalizedVersionId: "asset-1:v2",
            content: {
              mimeType: "image/png",
              sizeBytes: 100,
              checksum: {
    ...
    }, new Uint8Array([1, 2, 3]), "token-3");
    await client.authorizeDownload({ workspaceId: "workspace-1", assetId: "asset-1", purpose: "download" }, "token-4");
    await client.resolvePreview({ workspaceId: "workspace-1", assetId: "asset-1", preferredMimeTypes: ["image/webp"] }, "token-5");
    ...
    expect(requests.map((entry) => entry.method)).toEqual(["GET", "GET", "POST", "POST", "GET", "POST"]);
    expect(requests.map((entry) => entry.url)).toEqual([
      ...
      "http://127.0.0.1:8788/api/v1/assets/upload-sessions/session-1/content?workspaceId=workspace-1",
    ...
    expect(requests[2]?.body).toBe("1,2,3");
    expect(requests[3]?.body).toContain('"purpose":"download"');
    expect(requests[5]?.body).toContain('"storageInstanceId":"storage-1"');
