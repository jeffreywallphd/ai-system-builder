...
describe("docs foundation validation script", () => {
  ...
  it("detects missing category scope mapping heading in documentation quality standard", () => {
    const fixtureRoot = mkdtempSync(join(tmpdir(), "docs-foundation-validator-quality-standard-scope-"));
    cpSync(join(repoRoot, "docs"), join(fixtureRoot, "docs"), { recursive: true });
    ...
    expect(combinedOutput).toContain("[DOCUMENTATION_QUALITY_STANDARD_INVALID]");
    expect(combinedOutput).toContain(
      "documentation-quality-standard.md is missing required heading '## Documentation Category Rule Scope Matrix'.",
    );
  });
