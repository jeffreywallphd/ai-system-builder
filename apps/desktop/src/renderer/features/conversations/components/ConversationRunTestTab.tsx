...
export function ConversationRunTestTab({ workspaceId }: { workspaceId: string }) {
  ...
  const refreshPlans = async () => { const r = await plansClient.listPlanSummaries({ targetWorkspaceId: workspaceId }); if (r.ok) { const items = r.value.summaries ?? []; setPlans(items); if (!planId && items[0]?.planId) setPlanId(items[0].planId); } };
  const refreshSessions = async (pid: string) => { if (!pid) return; const r = await client.listConversationSessions({ workspaceId, sourceExecutionPlanId: pid }); if (r.ok) { const xs = r.value.sessions ?? []; setSessions(xs); if (!sessionId && xs[0]?.conversationSessionId) setSessionId(xs[0].conversationSessionId); } };
