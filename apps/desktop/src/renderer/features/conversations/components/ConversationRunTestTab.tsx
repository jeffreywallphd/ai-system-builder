...
import { createDesktopAssetCompositionClient } from '../../asset-composition/api/desktopAssetCompositionClient';
...
export function ConversationRunTestTab({ workspaceId }: { workspaceId: string }) {
  const plansClient = useMemo(() => createDesktopAssetCompositionClient(), []);
  ...
  return <section className="ui-stack ui-stack--sm"><h2>Run &amp; Test</h2>
    <div className="ui-panel ui-stack ui-stack--xs"><h3>Test an assistant</h3><p>Choose a conversational system from your assets to start a test conversation.</p>
      <label className="ui-stack ui-stack--sm"><span><TermWithHint termId="runPlan">Run plan</TermWithHint></span><select aria-label='Run plan' value={planId} onChange={(e)=>setPlanId(e.target.value)}><option value=''>Select a run plan</option>{plans.map((p:any)=><option key={p.planId ?? p.id} value={p.planId ?? p.id}>{p.name ?? p.planId ?? p.id}</option>)}</select></label>
      ...
      {sessions.length ? <label className="ui-stack ui-stack--sm"><span><TermWithHint termId="testConversation">Test conversation</TermWithHint></span><select aria-label='Test conversation' value={sessionId} onChange={(e)=>setSessionId(e.target.value)}>{sessions.map((s:any)=><option key={s.conversationSessionId ?? s.id} value={s.conversationSessionId ?? s.id}>{s.sessionLabel ?? s.systemLabel ?? (s.conversationSessionId ?? s.id)}</option>)}</select></label> : null}
    ...
    <section className='ui-panel ui-stack ui-stack--xs' aria-label='Conversation'>
      ...
      <label className="ui-stack ui-stack--sm"><span><TermWithHint termId="conversationMessage">Message</TermWithHint></span><textarea aria-label='Message' value={draft} onChange={(e)=>setDraft(e.target.value)} /></label>
