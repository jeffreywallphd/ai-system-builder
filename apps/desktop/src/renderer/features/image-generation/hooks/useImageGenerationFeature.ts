...
export interface ImageGenerationFinalizedAssetReference { assetId: string; artifactId: string; storageKey?: string; previewUrl?: string; mediaType?: string; previewStatus?: "idle" | "loading" | "ready" | "failed"; previewError?: string; }
export interface ImageGenerationSessionGeneration { id: string; requestId?: string; createdAt: string; assets: ImageGenerationFinalizedAssetReference[]; }
...
export interface ImageGenerationArtifactOption { value: string; label: string; }
...
export function useImageGenerationFeature(client?: DesktopImageGenerationClient, modelsClient?: DesktopModelsClient, artifactClient?: DesktopArtifactBrowserClient, workspaceId?: string) {
  ...
  const [form, setForm] = useState<ImageGenerationFormValues>({ prompt: "", negativePrompt: "", seed: "42", width: "1024", height: "1024", steps: "30", cfg: "8", denoise: "1", sampler: "euler", scheduler: "normal", model: "", numImages: "1", latentSourceArtifactId: "", faceIdEnabled: false, faceIdArtifactId1: "", faceIdArtifactId2: "", faceIdArtifactId3: "", faceIdIdentityStrength: "0.85", faceIdStructureStrength: "0.75", faceIdNoise: "0.35" });
  const [status, setStatus] = useState<ImageGenerationUiStatus>("idle"); const [message, setMessage] = useState<string | undefined>(undefined); const [error, setError] = useState<string | undefined>(undefined); const [requestId, setRequestId] = useState<string | undefined>(undefined); const [progress, setProgress] = useState<{ message?: string; current?: number; total?: number } | undefined>(undefined); const [taskData, setTaskData] = useState<ImageGenerationTaskDataView | undefined>(undefined); const [outputs, setOutputs] = useState<ImageGenerationOutputReference[]>([]); const [finalizedAssets, setFinalizedAssets] = useState<ImageGenerationFinalizedAssetReference[]>([]); const [sessionGallery, setSessionGallery] = useState<ImageGenerationSessionGeneration[]>([]); const [installStatus, setInstallStatus] = useState<RuntimeInstallStatus>("unknown");
  const [availableModels, setAvailableModels] = useState<ImageGenerationModelOption[]>([]); const [availableImageArtifacts, setAvailableImageArtifacts] = useState<ImageGenerationArtifactOption[]>([]); const [modelLoadStatus, setModelLoadStatus] = useState<ImageGenerationModelLoadStatus>("idle"); const [modelLoadMessage, setModelLoadMessage] = useState<string | undefined>(undefined); const [artifactLoadMessage, setArtifactLoadMessage] = useState<string | undefined>(undefined); const mountedRef = useRef(true); const activePollRef = useRef<string | undefined>(undefined); const installStatusSequenceRef = useRef(0);
  const isPollingStillActive = useCallback((id: string) => mountedRef.current && activePollRef.current === id, []);
  useEffect(() => () => {
    for (const url of previewUrlsRef.current) {
      URL.revokeObjectURL(url);
      recordRendererMemorySnapshot({ milestone: "renderer.preview.object-url.revoked", component: "desktop-renderer-preview-cleanup", detail: { pageKey: "image-generation", sectionKey: "image-generation.gallery", reason: "page-unmount" } });
    }
  ...
  const hydratePreviews = useCallback(async (assets: ImageGenerationFinalizedAssetReference[]) => {
    const loading = assets.map((a) => ({ ...a, previewStatus: "loading" as const }));
    const resolved = await Promise.all(loading.map(async (a) => {
      try { const viewUrl = await artifacts.createArtifactMediaViewUrl({ storageKey: a.storageKey ?? a.artifactId }); previewUrlsRef.current.push(viewUrl); return { ...a, previewUrl: viewUrl, previewStatus: "ready" as const }; }
      catch (e) { return { ...a, previewStatus: "failed" as const, previewError: e instanceof Error ? e.message : "Failed to load preview." }; }
    }));
    return resolved;
  }, [artifacts]);

  const replaceFinalizedAssets = useCallback((assets: ImageGenerationFinalizedAssetReference[], completedRequestId?: string) => {
  ...
  const poll = useCallback(async (id: string) => {
    ...
    try {
      while (isPollingStillActive(id)) {
        const read = await imageGenerationClient.readImageGeneration({ requestId: id, workspaceId } as never);
        ...
        if (s === "queued") { setStatus("queued"); await sleep(600); continue; }
        if (s === "running") { setStatus("running"); await sleep(600); continue; }
        if (s === "succeeded") {
          setStatus("finalizing");
          const fin = await imageGenerationClient.finalizeImageGenerationIfCompleted({ requestId: id, workspaceId } as never);
          if (!isPollingStillActive(id)) return;
          if (!fin.ok) { setStatus("failed"); setError(fin.error.message ?? "Failed to finalize generated images."); return; }
          const assets = (fin.value.assets ?? []).filter((a): a is ImageGenerationFinalizedAssetReference => typeof a?.assetId === "string" && typeof a?.artifactId === "string");
          if (!hasFinalizedAssets(assets)) { setStatus("failed"); setError("Image generation finalized but did not register any generated image artifacts."); return; }
          const resolvedAssets = await hydratePreviews(assets);
          replaceFinalizedAssets(resolvedAssets, id);
          setStatus("succeeded");
          return;
        }
        if (s === "failed") { setStatus("failed"); setError(read.value.error?.message ?? "Task failed."); return; }
        if (s === "cancelled") { setStatus("cancelled"); return; }
        setStatus("unknown");
        return;
      }
    } catch (cause) {
  ...
  }, [imageGenerationClient, isPollingStillActive, hydratePreviews, replaceFinalizedAssets, workspaceId]);
  const start = useCallback(async () => { if (validationError || isStartDisabled) return false; setStatus("starting"); setError(undefined); setMessage("Generating a new image. The previous result will stay visible until the new one is ready."); setTaskData(undefined); setOutputs([]); const payload: ImageGenerationRequest = { prompt: form.prompt, width: parsePositiveNumber(form.width), height: parsePositiveNumber(form.height), steps: parsePositiveNumber(form.steps), cfg: Number(form.cfg), denoise: Number(form.denoise), numImages: parsePositiveNumber(form.numImages), latentSource: form.latentSourceArtifactId.trim() ? { kind: "artifact", artifactId: form.latentSourceArtifactId.trim() } : { kind: "empty" } }; if (form.negativePrompt.trim()) payload.negativePrompt = form.negativePrompt.trim(); const seed = parseSeed(form.seed); if (seed !== undefined) payload.seed = seed; if (form.sampler.trim()) payload.sampler = form.sampler.trim(); if (form.scheduler.trim()) payload.scheduler = form.scheduler.trim(); if (form.model.trim()) payload.model = form.model.trim(); if (form.faceIdEnabled) { const references = [form.faceIdArtifactId1, form.faceIdArtifactId2, form.faceIdArtifactId3].map((value) => value.trim()).filter((value) => value.length > 0).slice(0,3).map((artifactId) => ({ artifactId })); if (references.length > 0) { payload.faceId = { enabled: true, references, identityStrength: Number(form.faceIdIdentityStrength), structureStrength: Number(form.faceIdStructureStrength), noise: Number(form.faceIdNoise) }; } } if (!workspaceId) { setStatus("failed"); setError("Select a workspace before generating images."); return false; }
    const started = await imageGenerationClient.startImageGeneration({ ...payload, workspaceId } as never); if (!started.ok || !started.value.requestId) { setStatus("failed"); setError(started.ok ? "Failed to start image generation." : started.error.message); return false; } setRequestId(started.value.requestId); setStatus("queued"); void poll(started.value.requestId); return true; }, [validationError, isStartDisabled, form, imageGenerationClient, poll, workspaceId]);
  const cancel = useCallback(async () => { if (!requestId) return; const cancelled = await imageGenerationClient.cancelImageGeneration({ requestId, workspaceId } as never); if (!isPollingStillActive(requestId)) return; if (cancelled.ok && cancelled.value.cancelled === true) { setStatus("cancelled"); setMessage(cancelled.value.message); activePollRef.current = undefined; return; } setMessage(cancelled.ok ? (cancelled.value.message ?? "Cancellation is not supported for this request.") : cancelled.error.message); }, [imageGenerationClient, isPollingStillActive, requestId, workspaceId]);
  const refreshInstallStatus = async () => { const sequence = ++installStatusSequenceRef.current; recordSectionLoadMilestone("renderer.section.load.start", { pageKey: "image-generation", sectionKey: "image-generation.runtime-readiness", trigger: "refresh" }); setInstallStatus("checking"); try { const result = await imageGenerationClient.readComfyUiInstallStatus?.({}); if (!mountedRef.current || installStatusSequenceRef.current !== sequence) return; if (!result || !result.ok) { setInstallStatus("unknown"); recordSectionLoadMilestone("renderer.section.load.failed", { pageKey: "image-generation", sectionKey: "image-generation.runtime-readiness", trigger: "refresh" }); return; } setInstallStatus(normalizeInstallStatus(result.value.status)); recordSectionLoadMilestone("renderer.section.load.resolved", { pageKey: "image-generation", sectionKey: "image-generation.runtime-readiness", trigger: "refresh" }); } catch { if (mountedRef.current && installStatusSequenceRef.current === sequence) setInstallStatus("unknown"); recordSectionLoadMilestone("renderer.section.load.failed", { pageKey: "image-generation", sectionKey: "image-generation.runtime-readiness", trigger: "refresh" }); } };
  ...
  return { form, setForm, status, requestId, message, error, progress, taskData, outputs, finalizedAssets, sessionGallery, validationError, isStartDisabled, start, cancel, refreshInstallStatus, repairInstall, installStatus, availableModels, availableImageArtifacts, artifactLoadMessage, modelLoadStatus, modelLoadMessage };
}
