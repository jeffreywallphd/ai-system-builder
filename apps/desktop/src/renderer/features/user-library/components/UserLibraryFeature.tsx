...
const toEffectiveSourceViewModel = (source: UserLibraryEffectiveSourceSummary) => ({ key: `${source.assetReference.assetId}:${source.assetReference.version}`, label: sourceLabel(source.effectiveSourceKind) });
...
export function UserLibraryFeature({ workspaceId }: { workspaceId: string }) {
  ...
  return <section><h2>Reusable Library</h2><p>Saved reusable assets</p><p><small>Promote/import flows are deferred in this minimal Phase 7 UI.</small></p>{loading ? <p>Loading reusable library data…</p> : null}{message ? <p>{message}</p> : null}<ul>{assets.length === 0 ? <li>No reusable assets yet.</li> : assets.map((a) => <li key={`${a.assetId}:${a.version}`}><strong>{a.title}</strong><button onClick={async () => { setMessage('Linking…'); const r = await client.link(workspaceId, { assetId: a.assetId, version: a.version }); setMessage(r.ok ? 'Linked to this workspace.' : 'Could not link this asset.'); await refresh(); }}>Link to this workspace</button><button onClick={async () => { setMessage('Copying…'); const r = await client.copy(workspaceId, { assetId: a.assetId, version: a.version }); setMessage(r.ok ? 'Copied into this workspace. Future library changes will not update this copy automatically.' : 'Could not copy this asset.'); await refresh(); }}>Copy into this workspace</button></li>)}</ul><h3>Linked to this workspace</h3><ul>{links.length === 0 ? <li>No links in this workspace.</li> : links.map((l) => <li key={l.id}>{l.title} — {l.propagationPolicy === 'explicit-update' ? 'Updates only when you choose to update' : 'Pinned version — no automatic updates'}</li>)}</ul><h3>What this workspace is using</h3><ul>{sources.length === 0 ? <li>No effective-source records yet.</li> : sources.map((s) => <li key={s.key}>{s.label}</li>)}</ul></section>;
}
