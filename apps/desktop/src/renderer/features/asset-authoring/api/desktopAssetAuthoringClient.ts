...
type Api = {
  ...
  createAssetDraft?: (i: { targetWorkspaceId: string; editableFields: { displayName: string; summary?: string; description?: string } }) => Promise<unknown>;
  updateAssetDraft?: (i: { targetWorkspaceId: string; draftId: string; editableFieldsPatch: { displayName?: string; summary?: string; description?: string } }) => Promise<unknown>;
...
export function createDesktopAssetAuthoringClient() {
  ...
  return {
    ...
    async createDraft(input: { workspaceId: string; displayName: string; summary?: string; description?: string }): Promise<Result<unknown>> {
      ...
      return parseEnvelope(await api.createAssetDraft({ targetWorkspaceId: input.workspaceId, editableFields: { displayName: input.displayName, summary: input.summary, description: input.description } }));
    },
    async updateDraft(input: { workspaceId: string; draftId: string; displayName?: string; summary?: string; description?: string }): Promise<Result<unknown>> {
      ...
      return parseEnvelope(await api.updateAssetDraft({ targetWorkspaceId: input.workspaceId, draftId: input.draftId, editableFieldsPatch: { displayName: input.displayName, summary: input.summary, description: input.description } }));
    },
    ...
    async listEffectiveSummaries(workspaceId: string): Promise<Result<{ items: readonly AssetAuthoringEffectiveSourceSummary[] }>> {
      ...
      const r = parseEnvelope<{ items?: readonly AssetAuthoringEffectiveSourceSummary[] } | { summaries?: readonly AssetAuthoringEffectiveSourceSummary[] }>(await api.listAssetAuthoringEffectiveSummaries({ targetWorkspaceId: workspaceId }));
      if (!r.ok) return r;
      return { ok: true, value: { items: 'items' in r.value ? (r.value.items ?? []) : (r.value.summaries ?? []) } };
    },
  };
}
