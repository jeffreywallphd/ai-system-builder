...
export function SystemBuilderPage({ workspaceId, workspaceName }: SystemBuilderPageProps) {
  ...
  return <section className="ui-stack ui-stack--sm" aria-labelledby="systems-title"><header className="ui-stack ui-stack--sm"><h1 id="systems-title">Systems</h1><p className="ui-text-muted">Build systems in {workspaceName} from reusable, versioned assets.</p></header><div className="asset-library-tabs" role="tablist" aria-label="System Builder sections"><button type="button" role="tab" aria-selected={activeTab === "compose"} onClick={() => setActiveTab("compose")}>Compose</button><button type="button" role="tab" aria-selected={activeTab === "plans"} onClick={() => setActiveTab("plans")}>Plans</button><button type="button" role="tab" aria-selected={activeTab === "build-release"} onClick={() => setActiveTab("build-release")}>Build &amp; Release</button><button type="button" role="tab" aria-selected={activeTab === "run-test"} onClick={() => setActiveTab("run-test")}>Run &amp; Test</button></div>{activeTab === "compose" ? <SystemBuilderWorkspace workspaceId={workspaceId} client={client} /> : null}{activeTab === "plans" ? <AssetPlansTab workspaceId={workspaceId} client={createThinClientAssetCompositionClient()} projectionClient={createThinClientEffectiveAssetProjectionsClient()} /> : null}{activeTab === "build-release" ? <SystemBuildReleaseWorkflow workspaceId={workspaceId} systemBuilderClient={client} buildClient={buildClient} defaultDeploymentProfile="thin-client" /> : null}{activeTab === "run-test" ? <div className="ui-stack ui-stack--md"><ConversationRunTestTab workspaceId={workspaceId} /><SystemDataRunTest workspaceId={workspaceId} client={dataClient} buildClient={buildClient} /></div> : null}</section>;
}
