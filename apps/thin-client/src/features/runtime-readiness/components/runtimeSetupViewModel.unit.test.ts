...
describe('runtimeSetupViewModel',()=>{
 ...
 it('sanitizes unsafe diagnostics',()=>{const vm=mapRuntimeSetup({status:'blocked',blockers:[{message:'token=abc'}],diagnostics:[{message:'Needs configuration'}],requirements:[],providerCandidates:[],bindings:[]},undefined); expect(vm.issues).toEqual(['Needs configuration']);});
});
