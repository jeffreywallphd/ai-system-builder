...
describe("thin-client useAssetLibraryFeature", () => {
  ...
  it("selecting a definition reads detail and exposes the overview fields", async () => {
    const client = createClient();
    ...
    expect(states.at(-1)?.selectedDetail?.overview?.description).toBe("Reusable document descriptor");
  });
